...
def _compile_rule(rule: dict) -> Optional[Tuple[re.Pattern, str]]:
    ...
    try:
        ...
        logger.warning(f"치환 규칙 컴파일 실패: pattern='{pattern_str}' comment='{comment}' error={e}")
        return None
...
def _load_and_parse(file_path: str) -> dict:
    ...
    try:
        ...
        logger.error(f"치환 설정 로드 실패: {file_path} error={e}")
        return result

    if not isinstance(data, dict):
        logger.error(f"치환 설정 형식이 잘못되었습니다. 딕셔너리여야 합니다: {file_path}")
        return result
