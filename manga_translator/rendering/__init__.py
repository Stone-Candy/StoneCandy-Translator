...
def _apply_region_font(region, fallback: str = '') -> None:
    font_value = getattr(region, 'font_family', '') or fallback or ''
    text_render.set_font(font_value or text_render.DEFAULT_FONT_FAMILY)
...
def _estimate_effect_padding(font_size: int, config: Config = None, stroke_width=None) -> float:
    ...
    if stroke_width is not None:
        stroke_ratio = _safe_float(stroke_width, stroke_ratio)
...
def _strip_layout_markup(text: str) -> str:
    """Remove layout markers/whitespace so pre/post line-break text can match."""
    text = re.sub(r'\s*(\[BR\]|【BR】|<br\s*/?>|\r\n|\r|\n)\s*', '', str(text or ''), flags=re.IGNORECASE)
    text = re.sub(r'</?H>', '', text, flags=re.IGNORECASE)
    # Layout may drop/add spaces around [BR]; compare on compact body text.
    text = re.sub(r'\s+', '', text)
    return text


def _sync_translation_raw_after_layout(region: TextBlock) -> None:
    """After layout inserts [BR]/<H>, keep translation_raw in sync.

    Normal first-time render leaves raw empty then copies translation (with
    markers) before YAML replacements. Import/load_text often pre-fills raw
    without markers, so the old ``if not raw`` guard skipped the copy and raw
    never received [BR]. When plain text matches, adopt the laid-out string.
    """
    laid_out = getattr(region, 'translation', None) or ''
    if not laid_out:
        return
    raw = getattr(region, 'translation_raw', None) or ''
    if not raw or _strip_layout_markup(raw) == _strip_layout_markup(laid_out):
        region.translation_raw = laid_out
...
def calc_text_block_dimensions(text: str, is_horizontal: bool, line_spacing: float = 1.0,
    ...
    """
    font_size = text_render.quantize_font_size(font_size)
    int_font = max(1, int(round(font_size)))
    scale = font_size / float(int_font)
    base_font = int_font
    prev_cw = text_render.get_char_width()
    text_render.set_char_width(char_width)
    try:
        ...
        if is_horizontal:
            normalized_lines = text_render.split_layout_lines(text_for_calc)
            if normalized_lines:
                widths = [
                ...
                base_width = max(widths, default=0) * scale
                base_height = (base_font * len(normalized_lines) + spacing_y * max(0, len(normalized_lines) - 1)) * scale
            ...
            if heights:
                ...
                base_width = (sum(line_widths) + spacing_x * max(0, len(lines) - 1)) * scale
                base_height = max(heights) * scale
                return base_width, base_height, len(lines)

        return 0, 0, 0
    finally:
        text_render.set_char_width(prev_cw)
...
def calc_font_from_box(width: float, height: float, text: str, is_horizontal: bool,
                       ...
                       char_width: float = 1.0, stroke_width=None) -> int:
    """
...
def calc_nearest_font_from_box(width: float, height: float, text: str, is_horizontal: bool,
                               line_spacing: float = 1.0, config: Config = None,
                               target_lang: str = None, letter_spacing: float = 1.0,
                               char_width: float = 1.0, stroke_width=None,
                               fit_axis: str | None = None) -> float:
    """에디터가 font_size로 레이어 크기를 만드는 calc_box_from_font를 역으로 푼다.

    fit_axis:
      - "height": 흰 박스 높이에 가장 가까운 값 (상하 핸들 / 가로쓰기 대각선)
      - "width": 흰 박스 폭에 가장 가까운 값 (좌우 핸들 / 세로쓰기 대각선)
      - None: 가로쓰기는 height, 세로쓰기는 width
    """
    if width <= 0 or height <= 0:
        return 1.0
    text = (text or "").strip()
    ...
    target = float(width if fit_axis == "width" else height)

    def _layer_size(fs: float):
    ...
    def _axis_size(fs: float) -> float:
        req_w, req_h, _ = _layer_size(fs)
        value = req_w if fit_axis == "width" else req_h
        if req_w <= 0.0 or req_h <= 0.0 or value <= 0.0:
            return 0.0
        return float(value)

    def _fits_axis(fs: float) -> bool:
        value = _axis_size(fs)
        return value > 0.0 and value <= target

    def _error(fs: float) -> float:
        value = _axis_size(fs)
        if value <= 0.0:
            return float("inf")
    ...
    hi = max(1, int(target))
    hi = min(hi, 8192)
    while hi < 8192 and _fits_axis(hi):
        lo = hi
        hi = min(hi * 2, 8192)
        if hi == lo:
            break
    while lo < hi:
        ...
        if _fits_axis(mid):
            ...
            hi = mid - 1
    floor_fs = max(1, int(lo))

    best_fs = float(floor_fs)
    best_err = _error(best_fs)
    step = text_render.FONT_SIZE_STEP
    candidate = floor_fs - 0.9
    while candidate <= floor_fs + 1.0 + 1e-9:
        fs = text_render.quantize_font_size(candidate)
        if fs >= 1.0:
            err = _error(fs)
            if err < best_err - 1e-9 or (abs(err - best_err) <= 1e-9 and fs > best_fs):
                best_fs = fs
                best_err = err
        candidate = round(candidate + step, 1)
    return text_render.quantize_font_size(best_fs)
...
def calc_box_from_font(font_size: int, text: str, is_horizontal: bool,
                       ...
                       char_width: float = 1.0, stroke_width=None) -> tuple:
    """
    ...
    base_w, base_h, n_lines = calc_text_block_dimensions(
        ...
        font_size=font_size, letter_spacing=letter_spacing, char_width=char_width,
    )
    ...
    effect_padding = _estimate_effect_padding(font_size, config, stroke_width=stroke_width)
...
def longer_opposite_edge_sizes(dst_points) -> tuple:
    """가로=윗/아랫변 중 긴 쪽, 세로=왼/오른변 중 긴 쪽."""
    pts = np.asarray(dst_points, dtype=np.float64).reshape(4, 2)
...
_DISTORT_DEST_RASTER_SCALE = 1.0


def distort_source_font_size(
    font_size: int,
    text: str,
    is_horizontal: bool,
    dst_points,
    ...
    """dest 긴 변의 2배 해상도로 글자를 다시 찍을 font_size. 저장된 레이아웃 글자 크기는 바꾸지 않는다."""
    font_size = max(1, int(font_size or 1))
    if not (text or "").strip():
        return font_size
    try:
        dest_w, dest_h = longer_opposite_edge_sizes(dst_points)
    except Exception:
        return font_size
    dest_w *= _DISTORT_DEST_RASTER_SCALE
    dest_h *= _DISTORT_DEST_RASTER_SCALE
    if dest_w <= 0.0 or dest_h <= 0.0:
        return font_size
    try:
        nat_w, nat_h, _ = calc_box_from_font(
            font_size,
            text,
            is_horizontal,
            line_spacing,
    ...
    if nat_w <= 0 or nat_h <= 0:
        return font_size
    scale = max(dest_w / float(nat_w), dest_h / float(nat_h), 1.0)
    if scale <= 1.0:
        return font_size
    long_edge = max(float(nat_w), float(nat_h)) * scale
    if long_edge > 32767.0:
        scale *= 32767.0 / long_edge
    return max(font_size, int(round(font_size * scale)))
...
def optimize_line_breaks_for_region(region: TextBlock, config: Config, target_font_size: int, bubble_width: float, bubble_height: float):
    ...
    for text_variant, combo_desc, skip_reason in combinations:
        ...
        if layout_mode == 'smart_scaling' and strict_smart_scaling:
            if '\n' not in text_for_calc:
                logger.debug(f"[OPTIMIZE_LINE_BREAKS] Skipping {combo_desc}: 엄격 스마트 스케일 모드에서 줄바꿈이 없으면 텍스트 박스가 커집니다")
                continue
    ...
    if optimized_br_count != original_br_count:
        ...
        if br_change > 0:
            ...
            change_desc = "调整了位置"
        logger.debug(f"[AI 줄바꿈 자동 확대] 최적화 완료：{change_desc} 개 줄바꿈, 글자 크기 {best_font_size:.1f}px")
        logger.debug(f"[AI 줄바꿈 자동 확대] 원문: {original_translation}")
        logger.debug(f"[AI 줄바꿈 자동 확대] 최적화 후: {best_text}")
        return best_text, best_font_size
    else:
        logger.debug(f"[AI 줄바꿈 자동 확대] 최적화하지 않음: 원래 줄바꿈이 더 나아 글자 크기 {best_font_size:.1f}px")
...
def _mask_bbox_size_in_text_frame(region: TextBlock, bubble_mask: np.ndarray) -> Optional[Tuple[float, float]]:
    """말풍선 마스크를 region 각도만큼 역회전시켜, 텍스트 렌더링 기준 좌표계에서의
    가로/세로 사용 가능 크기(bounding box)를 구한다.

    세로쓰기 원문 -> 가로쓰기 번역처럼 방향이 바뀌는 경우, region.unrotated_size
    (원본 OCR 라인박스)는 새 렌더링 방향과 종횡비가 맞지 않아 줄바꿈이 과도하게
    잘게 쪼개지는 문제가 있음. 이 함수는 실제로 채워야 할 마스크 공간을
    컨테이너 크기로 대신 쓰기 위한 값을 계산한다.
    """
    if bubble_mask is None or np.count_nonzero(bubble_mask) == 0:
        return None
    ...
    angle = _safe_float(getattr(region, 'angle', 0), 0.0)
    mask_u8 = np.where(bubble_mask > 0, 255, 0).astype(np.uint8)
    ...
    # calc_box_from_font 에서 dst_points = rotate_polygons(center, unrotated_points, -angle)
    # 로 만들어지므로, 그 역방향(마스크 -> 텍스트 기준 좌표계)은 +angle 회전에 해당.
    rot_mat = cv2.getRotationMatrix2D(center, angle, 1.0)
    rotated_mask = cv2.warpAffine(
        mask_u8, rot_mat, (w, h),
        flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0,
    )
    bx, by, bw, bh = cv2.boundingRect(rotated_mask)
    if bw <= 0 or bh <= 0:
        return None
    return float(bw), float(bh)
...
def _resolve_char_width_multiplier(region: TextBlock, config: Config) -> float:
    region_char_width = getattr(region, 'char_width', None)
    if isinstance(region_char_width, (int, float)) and region_char_width > 0:
        return float(region_char_width)
    cfg_val = config.render.char_width if config and hasattr(config, 'render') else None
    if isinstance(cfg_val, (int, float)) and cfg_val > 0:
        return float(cfg_val)
    return 1.0
...
def _resolve_original_font_size_cap(region: TextBlock, config: Config) -> int:
    """원본(OCR 감지) 폰트 크기 기준 상한을 구한다.

    마스크 기반 컨테이너 크기가 커져도, 원문 글씨 크기의 일정 배율을 넘지
    않도록 제한해서 폰트 크기가 과도하게 커지는 것("폭주")을 막는다.
    ``config.render.original_font_size_max_ratio``로 배율을 조절할 수 있고,
    지정하지 않으면 기본값 1.15배를 사용한다. 0 이하로 설정하면 캡을 끈다.
    """
    original_font_size = getattr(region, 'original_font_size', 0)
    if not (isinstance(original_font_size, (int, float)) and original_font_size > 0):
        return 0

    render_cfg = getattr(config, 'render', None) if config is not None else None
    ratio = 1.5
    if render_cfg is not None:
        ratio = _safe_float(getattr(render_cfg, 'original_font_size_max_ratio', 1.15), 1.15)
    if ratio <= 0:
        return 0

    return max(int(original_font_size * ratio), 1)
...
def _resolve_region_layout_center(
    ...
    _, req_h, _ = calc_box_from_font(
        ...
        region.target_lang,
        letter_spacing=letter_spacing_multiplier,
        char_width=_resolve_char_width_multiplier(region, config),
    )
    return _compute_top_aligned_center(region, req_h)


def _normalize_distort_quad(value) -> Optional[np.ndarray]:
    if value is None:
        return None
    try:
...
def _dst_points_from_distort_quad(region: TextBlock) -> Optional[np.ndarray]:
    if not bool(getattr(region, 'distortMode', False)):
        return None
    points = _normalize_distort_quad(getattr(region, 'distort_quad', None))
    if points is None:
        return None
    return points.reshape(1, 4, 2)


def _dst_points_from_custom_white_frame(region: TextBlock) -> Optional[np.ndarray]:
    """Use the editor custom white frame as the export dst when present."""
    distort_dst = _dst_points_from_distort_quad(region)
    if distort_dst is not None:
        return distort_dst
    if not bool(getattr(region, 'has_custom_white_frame', False)):
        return None
    wf = getattr(region, 'white_frame_rect_local', None)
    if not (isinstance(wf, (list, tuple)) and len(wf) == 4):
        return None
    try:
    ...
    # Build the frame in its local/unrotated orientation first, then apply
    # the same rotation convention used by calc_box_from_font().  The old
    # implementation only rotated the frame's CENTER and returned an
    # axis-aligned rectangle, which caused custom-white-frame regions to lose
    # their angle during final rendering.
    ...
    if abs(angle) < 1e-6:
        return unrotated_points.reshape(1, 4, 2)

    return rotate_polygons(
        (world_cx, world_cy),
        unrotated_points.reshape(1, -1),
        -angle,
        to_int=False,
    ).reshape(-1, 4, 2).astype(np.float32)


def _fit_font_size_to_white_frame(
    region: TextBlock,
    render_horizontally: bool,
    line_spacing_multiplier: float,
    letter_spacing_multiplier: float,
    ...
    width = max(0.0, right - left)
    height = max(0.0, bottom - top)
    text = (getattr(region, 'translation', None) or '').strip()
    if width <= 0.0 or height <= 0.0 or not text:
        return None
    try:
        fs = calc_font_from_box(
            ...
            line_spacing_multiplier,
            letter_spacing=letter_spacing_multiplier,
            char_width=float(getattr(region, 'char_width', 1.0) or 1.0),
        )
    except Exception:
        return None
    if fs <= 0:
...
def resize_regions_to_font_size(img: np.ndarray, text_regions: List['TextBlock'], config: Config, original_img: np.ndarray = None, return_debug_img: bool = False, skip_font_scaling: bool = False):
    ...
    logger.debug(f"[RESIZE] 처리 시작 {len(text_regions)} 개영역")
    ...
    for region_idx, region in enumerate(text_regions):
        if region is None:
            logger.info(f"[RESIZE] 영역 {region_idx}: None, 건너뜀")
        ...
        try:
            ...
            if not region.translation or not region.translation.strip():
                logger.info(f"[RESIZE] 영역 {region_idx}: translation이 비어 min_rect 사용")
            ...
            if distort_dst is not None:
                ...
                line_spacing_multiplier = _resolve_line_spacing_multiplier(region, config)
                letter_spacing_multiplier = _resolve_letter_spacing_multiplier(region, config)
                fitted = _fit_font_size_to_white_frame(
                    region,
                    actual_horizontal,
                    line_spacing_multiplier,
                ...
                if fitted is not None:
                    region.font_size = fitted
                dst_points_list.append(distort_dst)
                continue
            ...
            if skip_font_scaling:
                ...
                logger.debug(f"[RESIZE] skip_font_scaling: 영역 {region_idx} 고정 글자 크기 사용 {fixed_font_size}")
                ...
                custom_dst = _dst_points_from_custom_white_frame(region)
                if custom_dst is not None:
                    ...
                    if fitted is not None:
                        fixed_font_size = fitted
                    dst_points = custom_dst
                else:
                    dst_points = _calc_region_dst_points_for_font(
                        region=region,
            ...
            if mode == 'balloon_fill':
                ...
                try:
                    ...
                    if not lines_fully_enclosed:
                        ...
                        # 세로쓰기 원문(좁고 긴 세로 박스) -> 가로쓰기 번역처럼 방향이
                        # 실질적으로 뒤집히는 경우, 원본 OCR 라인박스를 그대로 가로 텍스트의
                        # 컨테이너로 쓰면 가로로 넓게 남는 마스크 공간을 못 쓰고 줄바꿈이
                        # 과도하게 잘게 쪼개짐. region.direction은 다른 강제 옵션(예:
                        # bubble_layout_english)에 의해 이미 'h'로 덮어써졌을 수 있어 믿을 수
                        # 없으므로, 실제 증상인 "원본 박스가 좁고 긴 세로형"인지 종횡비로 직접 판단.
                        narrow_tall_original_box = line_box_height > line_box_width * 1.2
                        direction_flipped_v_to_h = render_horizontally and narrow_tall_original_box
                        if direction_flipped_v_to_h:
                            mask_container_size = _mask_bbox_size_in_text_frame(region, region_bubble_mask)
                            if mask_container_size is not None:
                                mask_w, mask_h = mask_container_size
                                if mask_w > 0 and mask_h > 0:
                                    logger.debug(
                                        f"balloon_fill region {region_idx}: direction flipped v->h, "
                                        f"override line_box {line_box_width:.1f}x{line_box_height:.1f} "
                                        f"-> mask bbox {mask_w:.1f}x{mask_h:.1f}"
                                    )
                                    line_box_width, line_box_height = mask_w, mask_h
                        ...
                        if has_br:
                            if configured_fixed_font_size <= 0:
                                layout_target_font_size, box_fit_font_size = _select_preserved_line_layout_font(
                                    base_font_size=layout_target_font_size,
                                ...
                                layout_target_font_size = max(int(layout_target_font_size), layout_min_font_size)
                            else:
                                box_fit_font_size = 0
                            logger.debug(
                                f"balloon_fill region {region_idx}: keep explicit breaks, "
                                f"candidate font={layout_target_font_size}, box_fit={box_fit_font_size}"
                            )
                        else:
                        ...
                        # 원본 글씨 크기 기준 상한: 마스크(말풍선)가 커서 line_box가
                        # 커진 경우에도, 번역문 폰트가 원문 글씨 크기 대비 지나치게
                        # 커지는(=폭주하는) 것을 막는다. render.font_size로 고정값이
                        # 설정된 경우(configured_fixed_font_size > 0)는 사용자가 명시적으로
                        # 지정한 값이므로 캡을 적용하지 않는다.
                        if configured_fixed_font_size <= 0:
                            original_font_size_cap = _resolve_original_font_size_cap(region, config)
                            if original_font_size_cap > 0 and preferred_font_size > original_font_size_cap:
                                logger.debug(
                                    f"balloon_fill region {region_idx}: cap preferred_font_size "
                                    f"{preferred_font_size} -> {original_font_size_cap} "
                                    f"(original_font_size={getattr(region, 'original_font_size', 0)})"
                                )
                                preferred_font_size = max(int(original_font_size_cap), 1)
            ...
            if mode == 'strict':
                ...
                if is_replace_mode and len(region.lines) == 1 and not force_single_line_no_wrap:
                    logger.debug("[STRICT MODE] 치환 모드 한 줄 영역 방향 재작성, 자동 줄바꿈 허용")
                ...
                if use_ai_break:
                    ...
                    if force_single_line_no_wrap:
                        logger.debug("[STRICT MODE] 치환 모드 한 줄 강제 비개행 (OCR lines=1), 무한 크기 사용")
                        # 强制清洗文本：移除所有可能导致换行的字符（\n, [BR]等），确保它真的是单行
                        region.translation = re.sub(r'(\n|\[BR\]|【BR】|<br>)', '', region.translation, flags=re.IGNORECASE)
                    else:
                        logger.debug("[STRICT MODE] AI 줄바꿈이 켜져 무한 크기를 사용합니다")
                else:
            ...
            # --- Mode: smart_scaling ---
                ...
                try:
                    ...
                    # 根据有没有BR选择不同的计算方式
                    if has_br:
                        logger.debug(f"[SMART_SCALING] Region {region_idx}: BR 분기")
                        # 有BR：用无限宽度，按BR换行
                        required_width = 0
                        required_height = 0
                        ...
                        logger.debug(f"[SMART_SCALING] Region {region_idx}: BR 분기가 없어 줄바꿈을 역추론합니다")
                        (
                            region.translation,
                            target_font_size,
                            required_width,
...
async def dispatch(
    ...
    if config.render.renderer in (
        ...
        for _region in text_regions:
            if _region.translation:
                _sync_translation_raw_after_layout(_region)
    ...
    # Layout may have inserted [BR]/<H> into translation. Sync those markers into
    # translation_raw before optional YAML replacements (raw = pre-replacement + markers).
    for _region in text_regions:
        _sync_translation_raw_after_layout(_region)
    ...
    for region_idx, (region, dst_points) in enumerate(tqdm(zip(text_regions, dst_points_list), '[render]', total=len(text_regions))):
        ...
        try:
            # 检查是否有文本需要渲染
            if not region.translation or not region.translation.strip():
                logger.info(f"[RENDER] 빈 텍스트 영역을 건너뜁니다: text='{region.text[:20] if region.text else ''}', translation='{region.translation[:20] if region.translation else ''}'")
                continue
...
def render(
    ...
    render_font_size = region.font_size
    if bool(getattr(region, "distortMode", False)):
        render_font_size = distort_source_font_size(
            region.font_size,
            text_to_render,
            render_horizontally,
            dst_points,
    ...
    if use_hq_render:
        logger.debug(f"[HQ_RENDER] 고품질 렌더 모드 사용 (font_size={render_font_size})")
    ...
    box = None
    if bool(getattr(region, "distortMode", False)):
        # 4점 변형: dest에 맞게 다시 찍은 글자 비트맵을 여백 없이 워프한다.
        box = temp_box.copy()
    elif render_horizontally:
        if r_temp > r_orig:
