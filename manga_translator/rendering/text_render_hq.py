...
def render_text_with_upscale(
    ...
    **_unused_kwargs,
    ...
    if upscale_factor == 1:
        if is_horizontal:
            return text_render.put_text_horizontal(
                ...
                letter_spacing=letter_spacing, char_width=char_width,
    ...
    logger.debug(f"[HQ_RENDER] 사용 {upscale_factor}x 확대 렌더 (원래 글자 크기={font_size})")
    ...
    try:
        ...
        logger.error(f"[HQ_RENDER] 확대 렌더 실패: {e}，일반 렌더로 폴백")
        # 失败时回退到普通渲染
        if is_horizontal:
            return text_render.put_text_horizontal(
                font_size, text, width, height, alignment,
    ...
    if upscaled_canvas is None:
        logger.warning("[HQ_RENDER] 확대 렌더가 None을 반환했습니다")
        return None
    ...
    logger.debug(f"[HQ_RENDER] 렌더 완료: {original_w}x{original_h} -> {target_w}x{target_h}")
