...
class TextBlock(object):
    ...
    def __init__(self, lines: List[Tuple[int, int, int, int]],
        ...
        if font_size is None or font_size < 0:
            ...
            self.font_size = max(1.0, round(float(font_size) * 10.0) / 10.0)
        self.angle = angle
        ...
        self.region_id: str = str(kwargs.get('region_id') or '')
        self.bold: bool = bold
        ...
        # 编辑器自定义白框：导出/后端渲染时与编辑器预览共用同一尺寸基准
        self.has_custom_white_frame = bool(kwargs.get('has_custom_white_frame', False))
        wf = kwargs.get('white_frame_rect_local')
        self.white_frame_rect_local = (
            [float(v) for v in wf] if isinstance(wf, (list, tuple)) and len(wf) == 4 else None
        )
        self.distortMode = bool(kwargs.get('distortMode', False))
        distort_quad = kwargs.get('distort_quad')
        self.distort_quad = None
        if isinstance(distort_quad, (list, tuple)) and len(distort_quad) == 4:
            try:
                self.distort_quad = [
                    [float(p[0]), float(p[1])]
                    for p in distort_quad
                    if isinstance(p, (list, tuple)) and len(p) >= 2
                ]
                if len(self.distort_quad) != 4:
                    self.distort_quad = None
            except (TypeError, ValueError):
                self.distort_quad = None
    ...
    def to_dict(self):
        ...
        region_id = getattr(self, 'region_id', '') or ''
        if region_id:
