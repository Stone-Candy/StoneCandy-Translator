...
def save_pil_image(
    ...
    *,
    ...
    stealth_pngcomp: Optional[bytes] = None,
    ...
    **save_kwargs,
    ...
    If stealth_pngcomp (or a PNG/WebP stealth_source_path) is provided and the
    destination is PNG or lossless WebP, NovelAI stealth_pngcomp is re-embedded
    into the alpha LSB before writing.
    """
    from .stealth_pngcomp import apply_stealth_pngcomp_for_save
    ...
    try:
        ...
        image_to_save, stealth_image, stealth_kwargs = apply_stealth_pngcomp_for_save(
            image_to_save,
            target_format,
            payload=stealth_pngcomp,
            stealth_source_path=stealth_source_path,
        )
        save_kwargs.update(stealth_kwargs)
        lossless_webp = bool(stealth_kwargs.get('lossless') and target_format == 'WEBP')
        if lossless_webp:
            save_kwargs.pop('quality', None)

        if quality is not None and not lossless_webp and target_format in {'JPEG', 'WEBP', 'AVIF', 'HEIF'}:
        ...
        if stealth_image is not None and stealth_image is not image and stealth_image is not converted_image:
            stealth_image.close()
