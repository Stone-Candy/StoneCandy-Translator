    ...
    for idx, (image, config) in enumerate(images_with_configs):
        ...
        if isinstance(image, str):
            try:
                ...
                logger.error(f"  원본 이미지 로드 실패: {image_name} - {e}")
                ctx = Context()
        ...
        try:
            ...
            if not translated_path:
                logger.warning(f"  [건너뜀] 대응 번역 이미지를 찾지 못했습니다: {os.path.basename(image_name)}")
                ctx = Context()
                ctx.input = image
                ctx.image_name = image_name
                ctx.text_regions = []
            ...
            logger.info(f"  번역 이미지를 찾았습니다: {os.path.basename(translated_path)}")
            
            # === 步骤2: 对生肉图执行检测+OCR ===
            logger.info("  [1/4] 원본 검출+OCR...")
            # ✅ 检查停止标志
            await asyncio.sleep(0)
            translator._check_cancelled()
            ...
            if not raw_ctx.text_regions:
                logger.warning("  [건너뜀] 원본에서 텍스트 영역을 못 찾아 원본을 그대로 출력합니다")
                # 设置result为原图
            ...
            if not skip_to_save:
                ...
                logger.info(f"    원본 영역: {len(raw_ctx.text_regions)} -> 필터 후: {len(raw_regions_filtered)}")
                
                if not raw_regions_filtered:
                    logger.warning("  [건너뜀] 필터 후 유효 영역이 없어 원본을 그대로 출력합니다")
                    # 设置result为原图
                    raw_ctx.result = image
                    raw_ctx.text_regions = []
                    skip_to_save = True
            ...
            if not skip_to_save:
                ...
                if not translated_ctx.text_regions:
                    logger.warning("  [건너뜀] 번역 이미지에서 텍스트 영역을 못 찾아 원본을 그대로 출력합니다")
                    # 设置result为原图
                    raw_ctx.result = image
                    raw_ctx.text_regions = []
                    skip_to_save = True
            ...
            if not skip_to_save:
                ...
                logger.info(f"    번역 이미지 영역: {len(translated_ctx.text_regions)} -> 필터 후: {len(trans_regions_filtered)}")
                ...
                # === 步骤4: 区域匹配 ===
                logger.info("  [3/4] 영역 매칭...")
                # ✅ 检查停止标志
                await asyncio.sleep(0)
                translator._check_cancelled()
                
                logger.info(f"    원본 크기: {raw_size[0]}x{raw_size[1]}")
                logger.info(f"    번역 이미지 크기: {trans_size[0]}x{trans_size[1]}")
                logger.info(f"    배율: x={raw_size[0]/trans_size[0]:.3f}, y={raw_size[1]/trans_size[1]:.3f}")
                ...
                logger.info(f"    매칭 결과: {len(matches)} 쌍 영역 (겹침률 >= 0.3, 작은 박스 기준)")
                ...
                if unmatched_raw_indices:
                    logger.info(f"    [미매칭] {len(unmatched_raw_indices)} 개 원본 영역이 매칭되지 않아 복구하지 않습니다: {sorted(unmatched_raw_indices)}")
                
                logger.info(f"    최종 영역: {len(matched_regions)} 개 (렌더용), {len(inpaint_regions)} 개 (복구용)")
                ...
                if translator.verbose:
                    try:
                        ...
                        logger.info(f"    [DEBUG] 원본 박스 수: {len(raw_regions_filtered)}, 번역 박스 수: {len(scaled_trans_regions)}, 매칭 쌍 수: {len(matches)}")
                ...
                if not inpaint_regions:
                    logger.warning("  [건너뜀] 복구할 영역이 없어 원본을 저장합니다")
            ...
            if not skip_to_save:
                ...
                logger.info(f"    [디버그] 복구 모델 설정: {inpainter_model} (유형: {type(inpainter_model)})")
                ...
                if is_none_inpainter:
                    ...
                    logger.info("    [복구 모델=none] 번역 치환 전용 검출 모듈 사용...")
                    ...
                    try:
                        ...
                        logger.info(f"    검출 완료, 원본 마스크 픽셀: {np.count_nonzero(mask_raw)}, 정제 후 픽셀: {np.count_nonzero(mask_refined) if mask_refined is not None else 0}")
                        
                    except Exception as e:
                        logger.warning(f"    [경고] 번역 치환 전용 검출 실패: {e}，단순 팽창으로 폴백")
                        # 回退方案：简单膨胀
                ...
                if raw_ctx.mask is not None:
                    ...
                    if mask_dilation_offset > 0:
                        ...
                        logger.info(f"    마스크 추가 팽창: kernel_size={kernel_size}, offset={mask_dilation_offset}픽셀, iterations={iterations}")
                    else:
                        logger.info(f"    마스크 추가 팽창을 건너뜁니다 (offset={mask_dilation_offset})")
                
                # 根据修复模型配置决定是否执行修复
                if is_none_inpainter:
                    # 修复模型为 none，使用智能涂白（直接用蒙版涂白）
                    logger.info("    [복구 모델=none] 스마트 흰색 칠을 사용하고 복구 모델을 건너뜁니다")
                    # 直接用白色填充蒙版区域
                    ...
                    # 使用修复模型进行修复
                    logger.info(f"    복구 모델로 복구합니다: {inpainter_model}")
                ...
                if config.render.enable_template_alignment:
                    logger.info("  [5/5] 직접 붙여넣기 모드 - darken_blend2 합성")
                    ...
                    if dilation_pixels > 0:
                        ...
                        logger.info(f"    마스크 처리: 이진화 + 팽창(3x3 타원 커널, {iterations}회 반복, 설정={dilation_pixels}픽셀)")
                    else:
                        translated_mask = thres
                        logger.info("    마스크 처리: 이진화만(팽창 꺼짐)")
                    ...
                    logger.info("    darken_blend2 합성: 글자 추출 -> 영역 지우기 -> 겹쳐 합치기")
                    ...
                    if translator.verbose:
                        try:
                            ...
                            logger.info(f"    [DEBUG] 추출한 글자를 저장합니다: {debug_text_path}")
                        except Exception as e:
                            logger.warning(f"    [DEBUG] 디버그 이미지 저장 실패: {e}")
                    ...
                    # 原有逻辑：OCR + 重新渲染
                    logger.info("  [5/5] 렌더 모드 - OCR 결과로 글자를 다시 렌더합니다")
            ...
            if save_info:
                try:
                    ...
                    # 편집기 재내보내기용 last_export_dir 기록에 필요
                    try:
                        raw_ctx.final_output_dir = final_output_dir
                    except Exception:
                        pass
                    ...
                    if hasattr(raw_ctx, 'result') and raw_ctx.result is not None:
                        ...
                        logger.info(f"  -> 저장함: {os.path.basename(final_output_path)}")
                        ...
                        if not config.render.enable_template_alignment:
                            ...
                            # JSON 저장 여부와 무관하게, 실제 출력 폴더는 항상 패치
                            translator._patch_last_export_dir(image_name, final_output_dir)
                        else:
                            logger.info("  -> [직접 붙여넣기 모드] JSON과 inpainted 저장을 건너뜁니다")
                        ...
                        if hasattr(config, 'cli') and hasattr(config.cli, 'export_editable_psd') and config.cli.export_editable_psd:
                            ...
                            if not config.render.enable_template_alignment:
                                try:
                                    ...
                                    script_only = not getattr(cli_cfg, 'psd_launch_photoshop', False)
                                    ...
                                    logger.info(f"  -> ✅ [PSD] 편집 가능한 PSD를 내보냈습니다: {os.path.basename(psd_path)}")
                                except Exception as psd_err:
                                    logger.error(f"  PSD 내보내기 실패: {psd_err}")
                            else:
                                logger.info("  -> [직접 붙여넣기 모드] PSD 내보내기를 건너뜁니다")
                        
                except Exception as save_err:
                    logger.error(f"  저장 실패: {save_err}")
...
def find_translated_image(raw_image_path: str) -> Optional[str]:
    ...
    if not os.path.isdir(translated_dir):
        logger.warning(f"번역 이미지 디렉터리가 없습니다: {translated_dir}")
        return None
    ...
    try:
        ...
        logger.warning(f"맞는 번역 이미지를 찾지 못했습니다 '{raw_basename}.*'，디렉터리의 파일: {files_in_dir[:5]}{'...' if len(files_in_dir) > 5 else ''}")
    except Exception as e:
        logger.error(f"디렉터리 파일을 나열할 수 없습니다: {e}")
...
def match_regions(raw_regions: List[TextBlock], 
    ...
    for trans_idx, trans_rect in enumerate(trans_rects):
        ...
        if matched_raws:
            ...
            if len(matched_raws) > 1:
                ...
                logger.info(f"    [다대일] T{trans_idx} (텍스트=\"{trans_text[:20] if trans_text else ''}...\") "
    ...
    if unmatched_trans:
        logger.warning(f"    [경고] {len(unmatched_trans)} 개 번역 영역이 매칭되지 않았습니다:")
        for trans_idx in sorted(unmatched_trans):
            trans_rect = trans_rects[trans_idx]
            trans_text = translated_regions[trans_idx].text if hasattr(translated_regions[trans_idx], 'text') else ''
            logger.warning(f"      T{trans_idx}: 위치=({trans_rect[0]:.0f},{trans_rect[1]:.0f}), "
                         f"尺寸={trans_rect[2]:.0f}x{trans_rect[3]:.0f}, "
                         f"文本=\"{trans_text[:20] if trans_text else ''}...\"")
    
    logger.info(f"    매칭 결과: {len(matches)} 개 번역 영역이 유지됩니다 (겹침률 >= {iou_threshold}, 작은 박스 기준)")
...
def calculate_template_alignment_offset(raw_img: np.ndarray, 
    ...
    try:
        ...
        if template_size <= 0:
            ...
            logger.info(f"    [정렬] 템플릿 크기를 자동 계산합니다: {template_size} 픽셀 (이미지 크기: {zw}x{zh})")
        ...
        if cenx < 0 or ceny < 0 or cenx + template_size > zw or ceny + template_size > zh:
            ...
            if max_template_size < 100:
                logger.warning(f"이미지 크기 {zw}x{zh} 너무 작아 템플릿 매칭을 할 수 없어 기본 오프셋을 사용합니다 (0, 0)")
                return (0, 0)
            template_size = max_template_size
            cenx = zw // 2 - template_size // 2
            ceny = zh // 2 - template_size // 2
            logger.info(f"    [정렬] 템플릿 크기를 자동 조정합니다: {template_size} 픽셀")
        ...
        logger.info(f"    [정렬] 템플릿 매칭 완료: 수평 오프셋={horizontal_offset}, 수직 오프셋={vertical_offset}")
        
        return (horizontal_offset, vertical_offset)
        
    except Exception as e:
        logger.error(f"    [정렬] 템플릿 매칭 실패: {e}")
        traceback.print_exc()
        return (0, 0)
...
def _refine_mask_winpy(rgbimg, rawmask):
    ...
    try:
        ...
        logger.warning("pydensecrf 설치되지 않아 CRF 최적화를 건너뜁니다")
        return rawmask
