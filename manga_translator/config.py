...
class RenderConfig(BaseModel):
    ...
    """Qt font family (or family::style) used for rendering."""
    ...
    """Character horizontal width scale. Default is 1.0. Scales glyphs+letter spacing uniformly; stroke is drawn after scale."""
...
class CliConfig(BaseModel):
    ...
    overwrite: str = "ask"
    """ask = prompt when outputs exist, always = overwrite, skip = keep existing outputs."""
    ...
    psd_launch_photoshop: bool = False
    """After generating the JSX script, launch Photoshop to produce the PSD. Off by default (script only)."""
    ...
    combine_txt: bool = True
    """Write/read one combined TXT instead of per-page TXT files"""
    backup_combined_txt: bool = True
    """When combine_txt is on, also write combined_script_BACKUP.txt after export"""
    write_txt_help: bool = True
    """When TXT export finishes, write a localized help TXT in that folder"""

    @model_validator(mode="before")
    @classmethod
    def _normalize_overwrite(cls, data: Any):
        if not isinstance(data, dict):
            return data
        value = data.get("overwrite")
        if value is None:
            return data
        if isinstance(value, str):
            ...
            if mapped:
                data = dict(data)
                data["overwrite"] = mapped
                return data
        if value is True:
            data = dict(data)
            data["overwrite"] = "always"
        elif value is False:
            data = dict(data)
            data["overwrite"] = "skip"
        return data
...
class OcrConfig(BaseModel):
    ...
    novelai_ocr: str = 'none'
    """NovelAI mode primary OCR override. 'none' keeps the normal OCR model."""
    novelai_secondary_ocr: str = 'none'
    """NovelAI mode secondary OCR override. 'none' disables hybrid OCR for that run."""
    novelai_ocr_vl_language_hint: str = 'none'
    """NovelAI mode PaddleOCR-VL language hint override. 'none' keeps the OCR tab hint."""
...
class Config(BaseModel):
    ...
    """Signed pixel offset applied to the final bubble inpaint region. Positive expands, negative shrinks. Default 0."""
    inpaint_to_overlay_dilation_offset: int = 0
    """Signed pixel offset when converting an inpaint blob to paint overlay. Positive expands, negative shrinks. Default 0."""
