    ...
    async def _load(self, device: str, **kwargs):
        ...
        if not device.startswith('cuda') and device != 'mps':
            try:
                ...
                self.logger.info(f'ONNX 모델 사용 (CPU 최적화): {onnx_path}')
                ...
                self.backend = 'onnx'
                self.logger.info(f'ONNX Runtime 버전: {ort.__version__} (메모리 최적화 모드)')
                return
            except Exception as e:
                self.logger.warning(f'ONNX 로드 실패, PyTorch로 폴백: {e}')
    ...
    async def _infer(self, image: np.ndarray, mask: np.ndarray, config: InpainterConfig, inpainting_size: int = 1024, verbose: bool = False) -> np.ndarray:
        ...
        if hasattr(self, 'backend') and self.backend == 'onnx':
            try:
                ...
                self.logger.warning(f'ONNX추론 실패（{str(e)[:100]}）, 이번은 PyTorch로 폴백')
                # 降级：加载PyTorch模型（.ckpt 应该已经在初始化时下载）
                if not hasattr(self, 'model'):
                    self.logger.info('PyTorch 모델을 로드합니다...')
                    self.model = load_lama_mpe(self._get_file_path('inpainting_lama_mpe.ckpt'), device='cpu')
                    self.model.eval()
...
class LamaLargeInpainter(LamaMPEInpainter):
    ...
    async def _load(self, device: str, force_torch: bool = False):
        ...
        if not device.startswith('cuda') and device != 'mps' and not force_torch:
            try:
                ...
                if not os.path.isfile(onnx_path):
                    self.logger.info('ONNX 모델이 없어 받아야 합니다')
                    # 标记为未下载，触发下载
                ...
                if not os.path.isfile(ckpt_path):
                    self.logger.warning(f'예비 PyTorch 모델이 없습니다: {ckpt_path}')
                    self.logger.info('예비 PyTorch 모델을 받습니다...')
                    try:
                        # 临时标记为未下载，触发下载
                        ...
                        self.logger.info('예비 PyTorch 모델 다운로드 완료')
                    except Exception as download_error:
                        self.logger.warning(f'예비 모델 다운로드 실패: {download_error}')
                        self.logger.warning('ONNX 추론이 실패하면 PyTorch로 내릴 수 없습니다')
                ...
                self.backend = 'onnx'
                self.logger.info(f'ONNX Runtime 버전: {ort.__version__}')
                return
            except Exception as e:
                self.logger.warning(f'ONNX 로드 실패, PyTorch로 폴백: {e}')
        ...
        # ✅ 强制使用PyTorch或GPU模式
        if force_torch:
            self.logger.info('"PyTorch 강제" 옵션이 켜져 ONNX를 건너뜁니다')
        ...
        # 检查 .ckpt 文件是否存在
        if not os.path.isfile(ckpt_path):
            self.logger.info('PyTorch 모델(.ckpt)이 없어 받아야 합니다')
            # 标记为未下载，触发下载
            self._downloaded = False
            await self._download()
            self._downloaded = True
        ...
        if hasattr(self, 'backend') and self.backend == 'onnx':
            try:
                ...
                if not hasattr(self, 'model'):
                    ...
                    ckpt_path = self._get_file_path('lama_large_512px.ckpt')
                    if not os.path.isfile(ckpt_path):
                        self.logger.error(f'PyTorch 모델 파일이 없습니다: {ckpt_path}')
                        self.logger.error('ONNX 추론이 실패했고 PyTorch 모델도 없어 복구할 수 없습니다')
