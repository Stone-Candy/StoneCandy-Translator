...
class SakuraDict():
    def __init__(self, path: str, logger: logging.Logger) -> None:
        ...
        if not os.path.exists(path):
            self.logger.warning(f"사전 파일이 없습니다: {path}")
            return
    ...
    def detect_type(self, dic_path: str):
        ...
        with open(dic_path, encoding="utf8") as f:
            dic_lines = f.readlines()
        self.logger.debug(f"사전 유형 감지: {dic_path}")
        if len(dic_lines) == 0:
            return "unknown"
    ...
    def get_dict_str(self):
        ...
        if self.dict_str == "":
            ...
            try:
                ...
                self.logger.warning(f"사전 로드 실패: {e}")
                return ""
        return self.dict_str
    ...
    def get_dict_from_file(self, dic_path: str):
        ...
        if dic_type == "galtransl":
            ...
            self.logger.warning(f"알 수 없는 사전 유형: {dic_path}")
        return self.dict_str
...
class SakuraTranslator(CommonTranslator):
    ...
    def detect_and_caculate_repeats(self, s: str, threshold: int = _REPEAT_DETECT_THRESHOLD, remove_all=True) -> Tuple[bool, str, int, str]:
        ...
        for pattern_length in range(1, len(s) // 2 + 1):
            ...
            while i < len(s) - pattern_length:
                ...
                counts.append(count)
                if count >= threshold:
                    self.logger.warning(f"반복 패턴을 감지했습니다: {pattern}，반복 횟수: {count}")
                    repeated = True
                    if remove_all:
    ...
    def _preprocess_queries(self, queries: List[str]) -> List[str]:
        ...
        self.logger.debug(f'전처리 후 쿼리 텍스트：{queries}')
        return queries
    ...
    async def _check_translation_quality(self, queries: List[str], response: str) -> List[str]:
        ...
        async def _retry_translation(queries: List[str], check_func: Callable[[str], bool], error_message: str) -> str:
            ...
            for i in range(self._RETRY_ATTEMPTS):
                ...
                self.logger.warning(f'{error_message} 시도 횟수: {i + 1}。현재 파라미터 스타일：{self._current_style}。')
        ...
        if self._detect_repeats(''.join(queries), self._REPEAT_DETECT_THRESHOLD):
            self.logger.warning(f'요청 내용 자체가 기본 임계값을 넘는 반복을 포함합니다{self._REPEAT_DETECT_THRESHOLD}의 반복 내용。')
        ...
        if self._detect_repeats(response, actual_threshold):
            ...
            if response is None:
                self.logger.warning(f'모델 열화가 의심되어 {self._RETRY_ATTEMPTS}회 시도해도 해결되지 않아 한 줄씩 번역합니다.')
        ...
        if not self._check_align(queries, response):
            ...
            if response is None:
                self.logger.warning(f'원문과 번역문 줄 수가 맞지 않아 {self._RETRY_ATTEMPTS}회 시도해도 해결되지 않아 한 줄씩 번역합니다。')
                return await self._translate_single_lines(queries)
    ...
    def _check_align(self, queries: List[str], response: str) -> bool:
        ...
        if not is_aligned:
            self.logger.warning(f"줄 수 불일치 - 원문 줄 수: {len(queries)}，번역문 줄 수： {len(translations)}")
        return is_aligned
    ...
    async def _translate_single_lines(self, queries: List[str]) -> List[str]:
        ...
        for query in queries:
            ...
            if self._detect_repeats(response):
                self.logger.warning(f"한 줄 번역 결과에 반복이 있습니다: {response}，원문을 반환합니다。")
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str], ctx=None) -> List[str]:
        ...
        self.logger.info(f'Sakura현재 연결 주소: {self.api_base}')
        self.logger.debug(f'원문： {queries}')
        text_prompt = '\n'.join(queries)
        ...
        while True:
            try:
                ...
                if timeout_attempt >= self._TIMEOUT_RETRY_ATTEMPTS:
                    raise Exception('Sakura 시간 초과.')
                self.logger.warning(f'Sakura가 시간 초과로 재시도합니다. 시도 횟수： {timeout_attempt}')
            except openai.RateLimitError:
                ratelimit_attempt += 1
                if ratelimit_attempt >= self._RATELIMIT_RETRY_ATTEMPTS:
                    raise
                self.logger.warning(f'Sakura가 속도 제한으로 재시도합니다. 시도 횟수： {ratelimit_attempt}')
                await asyncio.sleep(2)
            except (openai.APIError, openai.APIConnectionError) as e:
                server_error_attempt += 1
                if server_error_attempt >= self._RETRY_ATTEMPTS:
                    self.logger.error(f'Sakura API 요청 실패. 주소：{self.api_base}, 오류 정보： {e}')
                    raise Exception(f'Sakura API 요청 실패 (주소: {self.api_base}): {e}') from e
                self.logger.warning(f'Sakura서버 오류로 재시도합니다。주소：{self.api_base}，시도 횟수： {server_error_attempt}，오류 정보： {e}')
