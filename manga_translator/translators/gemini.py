...
class GeminiTranslator(CommonTranslator):
    ...
    def _setup_client(self, system_instruction=None):
        ...
        if not self.client and self.api_key:
            ...
            if is_custom_api:
                ...
                self.logger.info(f"Gemini클라이언트 초기화 완료 (curl_cffi 강제, 사용자 API Base)。Base URL: {self.base_url}")
            else:
                self.client = AsyncGeminiCurlCffi(
                    api_key=self.api_key,
                    default_headers=BROWSER_HEADERS,
                ...
                self._use_curl_cffi = True
                self.logger.info("Gemini클라이언트 초기화 완료 (curl_cffi 강제)")
    ...
    async def _abort_inflight_request(self):
        ...
        try:
            ...
            self.logger.debug(f"Gemini 요청 중단 중 클라이언트 닫기 실패 (무시 가능): {e}")
        finally:
            self.client = None
    ...
    async def _translate_batch(self, texts: List[str], source_lang: str, target_lang: str, custom_prompt_json: Dict[str, Any] = None, line_break_prompt_json: Dict[str, Any] = None, ctx: Any = None, split_level: int = 0) -> List[str]:
        ...
        if not self.client:
            self.logger.error("Gemini클라이언트 초기화 실패")
            return texts
        ...
        while is_infinite or attempt < max_retries:
            ...
            try:
                ...
                if finish_reason and "STOP" not in finish_reason_str.upper():  # 不是成功
                    ...
                    self.logger.warning(f"Gemini API 실패 ({log_attempt}): {diagnostics_text}")
                    if gemini_diagnostics_indicate_safety(diagnostics) and not should_retry_without_safety:
                        self.logger.warning("감지Gemini안전 정책 차단, 다음 재시도에서 안전 설정 파라미터를 뺍니다。")
                ...
                if not result_text:
                    log_attempt = f"{attempt}/{max_retries}" if not is_infinite else f"Attempt {attempt}"
                    self.logger.warning(f"Gemini빈 내용을 반환했습니다 ({diagnostics_text}) ({log_attempt})。재시도합니다...")
                    if gemini_diagnostics_indicate_safety(diagnostics) and not should_retry_without_safety:
                        self.logger.warning("빈 응답과 함께 Gemini안전 정책 정보가 있어 다음 재시도에서 안전 설정 파라미터를 뺍니다。")
                        should_retry_without_safety = True
                ...
                if not self._validate_br_markers(translations, queries=texts, ctx=ctx):
                    ...
                    if not is_infinite and attempt >= max_retries:
                        ...
                        self.logger.error("Gemini 번역이 여러 번 재시도해도 실패했습니다: AI 줄바꿈 검사 실패.")
                ...
                log_attempt = f"{attempt}/{max_retries}" if not is_infinite else f"Attempt {attempt}"
                self.logger.warning(f"Gemini번역 오류 ({log_attempt}): {e}")

                if gemini_error_message_indicates_safety(error_message):
                    self.logger.warning("감지Gemini안전 정책 차단. 재시도합니다...")
                
                # 检查是否达到最大重试次数
                if not is_infinite and attempt >= max_retries:
                    self.logger.error("Gemini 번역이 여러 번 재시도해도 실패했습니다. 프로그램을 종료합니다.")
                    raise e
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str], ctx=None) -> List[str]:
        ...
        self.logger.info(f"Gemini 순수 텍스트 번역으로 처리합니다: {len(queries)}개 텍스트, 최대 시도: {self._max_total_attempts}")
