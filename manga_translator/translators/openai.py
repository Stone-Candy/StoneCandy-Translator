...
class OpenAITranslator(CommonTranslator):
    ...
    def _setup_client(self, force_recreate: bool = False):
        ...
        if not self.client:
            ...
            self.logger.debug("새 OpenAI 클라이언트를 만들었습니다 (curl_cffi 강제)")
        ...
        while is_infinite or attempt < max_retries:
            ...
            try:
                ...
                if has_content:
                    ...
                    if not self._validate_br_markers(translations, queries=texts, ctx=ctx):
                        ...
                        if not is_infinite and attempt >= max_retries:
                            ...
                            self.logger.error("OpenAI 번역이 여러 번 재시도해도 실패했습니다: AI 줄바꿈 검사 실패.")
                ...
                if finish_reason == 'content_filter':
                    ...
                    self.logger.warning(f"OpenAI 내용이 안전 정책에 걸렸습니다 ({log_attempt}). 재시도합니다...")
                    last_exception = Exception("OpenAI content filter triggered")
                elif finish_reason == 'length':
                    retry_reason = "Response truncated due to length limit"
                    self.logger.warning(f"OpenAI응답이 잘렸습니다 (토큰 한도) ({log_attempt})。재시도합니다...")
                    ...
                    self.logger.warning(f"OpenAI번역 대신 도구 호출을 시도했습니다 ({log_attempt})。재시도합니다...")
                    ...
                    self.logger.warning(f"OpenAI빈 내용을 반환했습니다 (finish_reason: '{finish_reason}') ({log_attempt})。재시도합니다...")
                    ...
                    self.logger.warning(f"OpenAI예상치 못한 종료 사유를 반환했습니다 '{finish_reason}' ({log_attempt})。재시도합니다...")
                ...
                log_attempt = f"{attempt}/{max_retries}" if not is_infinite else f"Attempt {attempt}"
                last_exception = e
                self.logger.warning(f"OpenAI번역 오류 ({log_attempt}): {e}")
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str], ctx=None) -> List[str]:
        ...
        self.logger.info(f"OpenAI 순수 텍스트 번역으로 처리합니다: {len(queries)}개 텍스트, 최대 시도: {self._max_total_attempts}")
