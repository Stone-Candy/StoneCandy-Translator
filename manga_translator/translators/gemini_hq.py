...
class GeminiHighQualityTranslator(CommonTranslator):
    ...
    def _setup_client(self, system_instruction=None):
        ...
        if not self.client and self.api_key:
            ...
            if is_custom_api:
                ...
                self._use_curl_cffi = True
                self.logger.info(f"{self._log_provider_name()}클라이언트 초기화 완료 (curl_cffi 강제)")

            self.logger.info("안전 설정: 기본 전송 OFF, 오류 시 자동 폴백")
    ...
    async def _abort_inflight_request(self):
        ...
        try:
            ...
            self.logger.debug(f"{self._log_provider_name()} 요청 중단 중 클라이언트 닫기 실패 (무시 가능): {e}")
        finally:
            self.client = None
    ...
    async def _translate_batch_high_quality(self, texts: List[str], batch_data: List[Dict], source_lang: str, target_lang: str, custom_prompt_json: Dict[str, Any] = None, line_break_prompt_json: Dict[str, Any] = None, ctx: Any = None, split_level: int = 0) -> List[str]:
        ...
        for i, data in enumerate(batch_data):
            image = data.get('image')
            if image is None:
                self.logger.debug(f"이미지[{i + 1}] 이미지 데이터가 없어 업로드를 건너뜁니다")
                continue
            ...
            try:
                ...
                self.logger.warning(f"이미지[{i + 1}] 처리 실패, 업로드 건너뜀: {image_error}")
                continue
        ...
        if not send_images:
            self.logger.info(f"쓸 이미지가 없어 {self._log_provider_name()}순수 텍스트 요청 모드를 사용합니다")
        ...
        while is_infinite or attempt < max_retries:
            ...
            if not self.client:
                self.logger.error(f"{self._log_provider_name()}클라이언트 초기화 실패")
                return texts
            ...
            try:
                ...
                if finish_reason and "STOP" not in finish_reason_str.upper():  # 不是成功
                    ...
                    self.logger.warning(f"{self._log_provider_name()} API 실패 ({log_attempt}): {diagnostics_text}")
                    
                    if gemini_diagnostics_should_disable_images(diagnostics):
                        self.logger.warning(f"감지{self._log_provider_name()}차단 또는 알 수 없는 종료 상태, 다음 재시도에서는 이미지를 보내지 않습니다。")
                        send_images = False
                    if gemini_diagnostics_indicate_safety(diagnostics) and not should_retry_without_safety:
                        self.logger.warning(f"감지{self._log_provider_name()}안전 정책 차단, 다음 재시도에서 안전 설정 파라미터를 뺍니다。")
                        should_retry_without_safety = True

                    if not is_infinite and attempt >= max_retries:
                        self.logger.error(f"{self._log_provider_name()} 번역이 여러 번 재시도해도 실패: {diagnostics_text}")
                        break
                ...
                if not result_text:
                    self.logger.warning(f"{self._log_provider_name()}빈 내용을 반환했습니다 ({diagnostics_text})，다음 재시도에서는 이미지를 보내지 않습니다")
                    send_images = False
                    if gemini_diagnostics_indicate_safety(diagnostics) and not should_retry_without_safety:
                        self.logger.warning(f"빈 응답과 함께 {self._log_provider_name()}안전 정책 정보가 있어 다음 재시도에서 안전 설정 파라미터를 뺍니다。")
                        should_retry_without_safety = True
                ...
                if not self._validate_br_markers(translations, batch_data=batch_data, ctx=ctx):
                    ...
                    if not is_infinite and attempt >= max_retries:
                        ...
                        self.logger.error(f"{self._log_provider_name_zh()}여러 번 재시도해도 실패했습니다: AI 줄바꿈 검사 실패。")
                ...
                if is_502_error or is_safety_error or is_empty_content:
                     if is_empty_content:
                         self.logger.warning(f"빈 응답을 감지해 다음 재시도에서는 이미지를 보내지 않습니다. 오류 정보: {error_message}")
                     else:
                         self.logger.warning(f"네트워크 오류(502) 또는 안전 설정 오류로 다음 재시도에서는 이미지를 보내지 않습니다。오류 정보: {error_message}")
                     send_images = False

                if is_bad_request and is_multimodal_unsupported:
                    self.logger.error(f"❌ 모델 {self.model_name} 이(가) 멀티모달 입력(이미지+텍스트)을 지원하지 않습니다")
                    self.logger.error("💡 해결 방법：")
                    self.logger.error(f"   1. 멀티모달을 지원하는 {self._log_provider_name()} 모델을 사용하세요")
                    self.logger.error("   2. 또는 일반 번역 모드로 바꾸세요 (고품질 번역기 없이)")
                    self.logger.error("   3. 서드파티 API가 이미지 입력을 지원하는지 확인하세요")
                    raise Exception(f"모델이 멀티모달 입력을 지원하지 않습니다: {self.model_name}") from e
                
                # 如果是安全设置错误且还没有尝试回退，则标记回退
                if is_safety_error and not should_retry_without_safety:
                    self.logger.warning(f"안전 설정 관련 오류를 감지해 다음 재시도에서 해당 파라미터를 뺍니다: {error_message}")
                    should_retry_without_safety = True
                    # 不增加attempt计数，直接重试
                    await self._sleep_with_cancel_polling(1)
                    continue
                ...
                log_attempt = f"{attempt}/{max_retries}" if not is_infinite else f"Attempt {attempt}"
                self.logger.warning(f"{self._log_provider_name_zh()} 오류 ({log_attempt}): {e}")

                if gemini_error_message_indicates_safety(error_message):
                    self.logger.warning(f"감지{self._log_provider_name()}안전 정책 차단. 재시도합니다...")
                    send_images = False # 显式确保降级
                
                # 检查是否达到最大重试次数
                if not is_infinite and attempt >= max_retries:
                    self.logger.error(f"{self._log_provider_name()} 번역이 여러 번 재시도해도 실패했습니다. 프로그램을 종료합니다.")
                    raise e
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str], ctx=None) -> List[str]:
        ...
        if not batch_data:
            ...
            self.logger.info(f"{self._log_provider_name()}batch_data가 없어 통합 대체 배치 경로를 사용합니다")
