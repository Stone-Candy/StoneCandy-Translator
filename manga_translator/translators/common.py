...
class BRMarkersValidationException(Exception):
    ...
    def __init__(self, missing_count: int, total_count: int, tolerance: int):
        ...
        super().__init__(
            f"AI 줄바꿈 검사 실패: {missing_count}/{total_count}개 번역에 [BR] 표시가 없습니다 (허용치: {tolerance})"
        )
    ...
    class Models:
        ...
        async def list(self):
            ...
            if 'application/json' not in content_type and 'text/json' not in content_type:
                ...
                raise Exception("API가 모델 목록 조회를 지원하지 않습니다(JSON이 아닌 응답). 모델 이름을 직접 입력하세요.")

            try:
                result = response.json()
            except Exception as e:
                raise Exception(f"API 응답을 해석할 수 없습니다: {str(e)}. 모델 이름을 직접 입력하세요.")
    ...
    class Models:
        ...
        async def generate_content(self, model, contents, generation_config=None, safety_settings=None, **kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"API 응답을 해석할 수 없습니다: {str(e)}. 응답 내용: "
                    f"{summarize_response_text(response.text)}"
                )
...
def validate_openai_response(response, logger=None) -> bool:
    ...
    if not hasattr(response, 'choices'):
        error_msg = f"API가 잘못된 응답 객체를 반환했습니다: {type(response).__name__}, 내용: {str(response)[:200]}"
        if logger:
            logger.error(error_msg)
        raise Exception(f"API가 잘못된 응답 객체를 반환했습니다. 유형: {type(response).__name__}")
...
def validate_gemini_response(response, logger=None) -> bool:
    ...
    if not hasattr(response, 'candidates'):
        error_msg = f"Gemini API가 잘못된 응답 객체를 반환했습니다: {type(response).__name__}, 내용: {str(response)[:200]}"
        if logger:
            logger.error(error_msg)
        raise Exception(f"Gemini API가 잘못된 응답 객체를 반환했습니다. 유형: {type(response).__name__}")
    
    # 检查是否有text属性（某些错误响应可能没有）
    if not hasattr(response, 'text'):
        diagnostics = extract_gemini_response_diagnostics(response)
        error_msg = f"Gemini API 응답에 text 속성이 없습니다: {format_gemini_response_diagnostics(diagnostics)}"
        if logger:
            logger.error(error_msg)
        raise Exception("Gemini API 응답에 text 속성이 없습니다")
        ...
        try:
            ...
            if not task.done():
                ...
                with contextlib.suppress(Exception):
                    ...
                    if not done:
                        self.logger.debug("취소 요청이 시간 초과되어 대기를 끝냅니다")
            if on_cancel:
                try:
                    ...
                    self.logger.debug("취소 중 정리 요청이 시간 초과되어 대기를 끝냅니다")
                except Exception as cleanup_error:
                    self.logger.debug(f"취소 중 정리 요청 실패 (무시 가능): {cleanup_error}")
            raise
...
**FINAL INSTRUCTION:** Translate the provided text regions faithfully and follow the separate output-format requirements appended below."""
    ...
    def _build_system_prompt_without_glossary(
        ...
        if output_format_prompt:
            ...
            self.logger.info("자동 용어 추출이 꺼져 있고 표준 출력 포맷 프롬프트도 없습니다。")
        return final_prompt
    ...
    def _build_system_prompt_with_glossary(
        ...
        if glossary_sections:
            ...
            self.logger.info("자동 용어 추출이 켜져 new_terms 출력 포맷 시스템 프롬프트를 사용합니다。")
        else:
            self.logger.info("자동 용어 추출이 켜져 있지만 추가 프롬프트를 못 불러왔습니다。")
    ...
    def _build_unified_user_prompt(self, batch_data: List[Dict], ctx=None, prev_context: str = "", retry_attempt: int = 0, retry_reason: str = "", is_image_mode: bool = True) -> str:
        ...
        for img_idx, data in enumerate(batch_data):
            ...
            for region_idx, text in enumerate(data['original_texts']):
                # 跳过 None 值
                if text is None:
                    self.logger.warning(f"None 텍스트를 건너뜁니다 (img_idx={img_idx}, region_idx={region_idx})")
                    continue
    ...
    def _validate_br_markers(self, translations: List[str], queries: List[str] = None, ctx=None, batch_indices: List[int] = None, batch_data: List = None, split_level: int = 0) -> bool:
        ...
        if split_level >= 3:
            self.logger.info(f"[AI 줄바꿈 검사] 분할 단계가 너무 깊습니다 (split_level={split_level})，BR 표시 검사를 건너뜁니다")
            return True
        ...
        if needs_check_count > 0:
            ...
            if missing_br_count > tolerance:
                ...
                # 全部通过
                self.logger.info(f"[AI 줄바꿈 검사] ✓ 모든 여러 줄 영역 번역에 [BR]이 있습니다 (검사 {needs_check_count}/{len(translations)} 개)")
                return True
...
def sanitize_text_encoding(text: str) -> str:
    ...
    try:
        ...
        logger.warning(f"텍스트 인코딩 정리 실패: {e}，원본 텍스트를 반환합니다")
        # 如果清理失败，至少移除明显的问题字符
