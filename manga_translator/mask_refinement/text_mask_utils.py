...
def _complete_mask_with_det_rearrange(
    ...
    # reconstructed reliably from a clipped local view. Process these rare
    # seam-crossing lines in a small context crop, then merge them back.
...
def _complete_mask_core(img: np.ndarray, mask: np.ndarray, textlines: List[Quadrilateral], keep_threshold = 1e-2, dilation_offset = 0, kernel_size=3, show_progress=True):
