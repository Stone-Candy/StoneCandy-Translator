...
# Final bubble inpaint inset/expand is applied via bubble_mask_dilation_offset.
BUBBLE_MASK_ERODE_PX = 0
...
def morph_binary_by_offset(mask: np.ndarray, offset_px: int) -> np.ndarray:
    """Expand (positive) or shrink (negative) a binary mask by |offset_px| pixels.

    Uses an elliptical kernel of size (2 * |offset| + 1), matching the editor
    inpaint-to-overlay conversion.
    """
    if mask is None:
        return mask
    offset = int(offset_px)
    if offset == 0:
        return mask
    size = abs(offset) * 2 + 1
    ...
    return np.where(result > 0, 255, 0).astype(np.uint8)


def _apply_bubble_inpaint_dilation_offset(
    refined_mask: np.ndarray,
    bubble_mask: np.ndarray,
    offset_px: int,
) -> np.ndarray:
    """Morph only the final bubble inpaint pixels; leave outside (SFX) regions unchanged."""
    offset = int(offset_px)
    if offset == 0 or refined_mask is None or bubble_mask is None:
        return refined_mask
    bubble_bin = bubble_mask > 0
    if not np.any(bubble_bin):
        return refined_mask
    bubble_inpaint = np.where((refined_mask > 0) & bubble_bin, 255, 0).astype(np.uint8)
    outside = np.where((refined_mask > 0) & ~bubble_bin, 255, 0).astype(np.uint8)
    return cv2.bitwise_or(outside, morph_binary_by_offset(bubble_inpaint, offset))
...
async def dispatch(
    ...
    bubble_mask_dilation_offset: int = 0,
    ...
    bubble_mask_dilation_offset = int(bubble_mask_dilation_offset)
    if use_model_bubble_repair_intersection or limit_mask_dilation_to_bubble_mask or bubble_mask_dilation_offset != 0:
        try:
            ...
            if np.count_nonzero(bubble_mask) > 0 and bubble_mask_dilation_offset != 0:
                mask_before_offset = final_mask
                final_mask = _apply_bubble_inpaint_dilation_offset(
                    refined_mask=final_mask,
                    bubble_mask=bubble_mask,
                    offset_px=bubble_mask_dilation_offset,
                )
                logger.info(
                    f"Bubble inpaint dilation offset={bubble_mask_dilation_offset}: "
                    f"before={int(np.count_nonzero(mask_before_offset))}, "
                    f"after={int(np.count_nonzero(final_mask))}"
                )
