...
def ensure_custom_api_params_file(path: str | None = None, logger=None) -> str:
    ...
    if logger:
        logger.info(f"Created custom API params file: {config_path}")
    return config_path
...
def load_custom_api_params_file(logger, path: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(params, dict):
            logger.error(f"Custom API params file must be a JSON object: {config_path}")
            return {}
        logger.info(f"Loaded custom API params: {params}")
        return params
    except Exception as exc:
        logger.error(f"Failed to load custom API params: {exc}")
    return {}
...
def load_enabled_custom_api_params(
    ...
    if resolved:
        logger.info(f"Enabled custom API params [{target}]: {resolved}")
    return resolved
...
def load_custom_api_params_sections(
    ...
    if any(normalized.values()):
        logger.info(f"Enabled categorized custom API params: {normalized}")
    return normalized
