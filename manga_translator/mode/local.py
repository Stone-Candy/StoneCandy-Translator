...
def _should_skip_existing_cli_file(file_path, cli_config, translator=None, save_info=None):
    ...
    if cli_config.get('translate_json_only', False):
        from manga_translator.utils.path_manager import (
            COMBINED_SCRIPT_TXT_NAME,
            find_script_txt_for_mode,
        )
        combine = bool(cli_config.get('combine_txt', True))
        txt_path = find_script_txt_for_mode(file_path, combine)
        if not txt_path:
            name = COMBINED_SCRIPT_TXT_NAME if combine else "script.txt"
            return True, f"原文文件不存在: {name}"
        return False, ""
...
async def translate_files(input_paths, output_dir, config_service, verbose=False, overwrite=False, args=None):
    ...
    if overwrite:
        ...
        mode = str(cli_config.get('overwrite', 'skip')).strip().lower()
        overwrite = mode in {'always', 'true', 'overwrite', '2'}
    ...
    rerender_only = cli_config.get('rerender_only', False)
    ...
    if cli_config.get('batch_concurrent', False) and has_incompatible_mode:
        ...
        print(f"⚠️  동시 파이프라인이 비활성화되었습니다: 현재 모드 [{', '.join(incompatible_modes)}] 동시 처리를 지원하지 않습니다")
        cli_config['batch_concurrent'] = False
        config_dict['cli'] = cli_config
    
    print(f"\n{'='*60}")
    print(f"번역기: {config_dict['translator']['translator']}")
    print(f"대상 언어: {config_dict['translator']['target_lang']}")
    print(f"사용 GPU: {cli_config.get('use_gpu', True)}")
    print(f"ONNX GPU 끄기: {cli_config.get('disable_onnx_gpu', False)}")
    print(f"배치 크기: {cli_config.get('batch_size', 1)}")
    print(f"동시 모드: {'사용' if cli_config.get('batch_concurrent', False) else '비활성화'}")
    print(f"기존 파일 덮어쓰기: {overwrite}")
    print(f"출력 형식: {cli_config.get('format') or '원본 형식 유지'}")
    print(f"저장 품질: {cli_config.get('save_quality', 95)}")
    print(f"{'='*60}\n")
    ...
    print(f"📁 찾음 {len(all_files)} 개 이미지 파일\n")
    ...
    print(f"📤 출력 디렉터리: {final_output_dir}\n")
    ...
    for file_path in all_files:
        ...
        try:
            if os.path.exists(file_path) and os.path.isfile(file_path):
                ...
                print(f"❌ 파일이 없습니다: {os.path.basename(file_path)}")
        except Exception as e:
            print(f"❌ 접근할 수 없습니다: {os.path.basename(file_path)} - {e}")
    ...
    if not os.path.exists(final_output_dir):
        os.makedirs(final_output_dir, exist_ok=True)
        print(f"✅ 출력 디렉터리 생성: {final_output_dir}")
        ...
        for file_path, config in file_paths_with_configs:
            try:
                ...
                if should_skip:
                    skipped_count += 1
                    if verbose:
                        print(f"⏭️  건너뜀 - {skip_reason}")
                else:
                    filtered_file_paths.append((file_path, config))
            except Exception as e:
                # 如果检查失败，默认保留
                if verbose:
                    logger.debug(f"파일 확인 중 오류 {file_path}: {e}")
                filtered_file_paths.append((file_path, config))
        
        if skipped_count > 0:
            print(f"⏭️  {skipped_count}개 기존 파일을 건너뛰었습니다 (덮어쓰기 감지 꺼짐)")
            print("ℹ️  이 파일을 다시 번역하려면 --overwrite 를 사용하세요")
            file_paths_with_configs = filtered_file_paths
        else:
            print("✅ 기존 파일이 없어 모두 처리합니다")
            
    if not file_paths_with_configs:
        print("✅ 모든 파일을 건너뛰어 처리할 것이 없습니다")
        print(f"\n{'='*60}")
        print(f"✅ 성공 (건너뜀): {skipped_count}")
        print("❌ 실패: 0")
        print(f"📊 합계: {len(all_files)}")
        print(f"{'='*60}")
        return
    ...
    print(f"\n📊 일괄 처리 모드: 총 {total_images} 장 이미지, {total_batches}개 배치로 처리")
    print("📋 설정 저장:")
    print(f"   출력 디렉터리: {final_output_dir}")
    print(f"   출력 형식: {output_format or '원본 형식 유지'}")
    print(f"   덮어쓰기 모드: {overwrite}")
    print(f"   저장 품질: {cli_config.get('save_quality', 95)}")
    print(f"   배치 크기: {batch_size} 장/배치")
    ...
    try:
        ...
        if use_concurrent:
            ...
            print(f"⏳ 동시 파이프라인 모드: 이미지 {total_images}장을 한 번에 처리합니다...")
            logger.info(f"동시 일괄 번역 시작，save_info={save_info}")
            ...
            for file_path, config in file_paths_with_configs:
                ...
                try:
                    ...
                    print(f"❌ 열 수 없습니다: {os.path.basename(file_path)} - {e}")
            ...
            print("⏳ 일괄 번역 시작 (백엔드가 배치로 이미지를 로드합니다)...")
            logger.info(f"일괄 번역 시작，save_info={save_info}")
            if total_images > 0:
        ...
        print("\n📊 번역 완료, 결과 확인...\n")
        logger.info(f"수신 {len(contexts)} 개 번역 결과")
        ...
        for ctx in contexts:
            if ctx:
                ...
                if hasattr(ctx, 'translation_error') and ctx.translation_error:
                    failed_count += 1
                    print(f"❌ 번역 실패: {os.path.basename(ctx.image_name)}")
                    if verbose:
                        print(f"   오류: {ctx.translation_error}")
                elif hasattr(ctx, 'success') and ctx.success:
                    # 优先检查 success 标志（因为 result 可能被清理了）
                    success_count += 1
                    print(f"✅ 완료: {os.path.basename(ctx.image_name)}")
                elif ctx.result:
                    success_count += 1
                    print(f"✅ 완료: {os.path.basename(ctx.image_name)}")
                else:
                    failed_count += 1
                    print(f"❌ 번역 실패: {os.path.basename(ctx.image_name)} - 번역 결과가 비어 있습니다")
            else:
                failed_count += 1
                print("❌ 번역 실패: 알 수 없는 이미지")
        
        if failed_count > 0:
            print(f"\n⚠️ 일괄 번역 완료: 성공 {success_count}/{total_images} 장, 실패 {failed_count}/{total_images} 장")
        else:
            print(f"\n✅ 일괄 번역 완료: 성공 {success_count}/{total_images} 장")
        print(f"💾 파일을 저장했습니다:：{final_output_dir}")
    ...
    print(f"\n{'='*60}")
    if skipped_count > 0:
        print(f"✅ 성공: {success_count} (그리고 {skipped_count} 개 건너뜀)")
    else:
    ...
    if os.path.exists(final_output_dir):
        ...
        print(f"\n📁 출력 디렉터리: {final_output_dir}")
        print(f"   포함 {len(output_files)} 개 파일")
        if verbose and output_files:
            ...
            if len(output_files) > 10:
                print(f"   ... 나머지 {len(output_files) - 10}개 파일")
    else:
        print(f"\n⚠️  출력 디렉터리가 없습니다: {final_output_dir}")
    print()
...
async def run_local_mode(args):
    ...
    if config_path:
        if not config_service.load_config_file(config_path):
            print(f"❌ 설정 파일을 로드할 수 없습니다: {config_path}")
            sys.exit(1)
    ...
    if use_subprocess:
                ...
                for file_path in all_files:
                    try:
                        ...
                        if should_skip:
                            skipped_count += 1
                            if verbose:
                                print(f"⏭️  이미 있어 건너뜀: {os.path.basename(file_path)}")
                        else:
        ...
        try:
            ...
            if skipped_count > 0:
                print(f"✅ 성공: {success_count} (그리고 {skipped_count} 개 건너뜀)")
                print(f"❌ 실패: {failed_count}")
                print(f"📊 합계: {len(all_files) + skipped_count}")
            else:
                print(f"✅ 성공: {success_count}")
                print(f"❌ 실패: {failed_count}")
                print(f"📊 합계: {len(all_files)}")
