...
def is_color_image(image, threshold: float = 0.05) -> bool:
    ...
    if min_corr > 0.995:
        logger.debug(f"컬러 검출 - 채널 상관이 높음({min_corr:.4f}), 판단: 흑백")
        return False
    ...
    logger.debug(f"컬러 검출 - 채도: {mean_saturation:.4f}, 채널 표준편차 차이: {std_diff:.2f}, 최소 상관: {min_corr:.4f}, 판단: {'컬러' if is_color else '흑백'}")
...
class MangaJaNaiUpscaler(OfflineUpscaler):
    ...
    def _select_best_model(self, img) -> str:
        ...
        if is_color:
            ...
            logger.info(f"컬러 이미지를 감지해 IllustrationJaNai 모델을 사용합니다: {model_file}")
            return model_file
        ...
        logger.info(f"흑백 이미지를 감지했습니다 (해상도 {res}p)，MangaJaNai 모델 사용: {best_model}")
        return best_model
    ...
    async def _load_specific_model(self, filename: str, device: str):
        ...
        try:
            ...
            logger.warning("spandrel이 없어 모델을 수동으로 로드합니다")
            try:
