...
class YOLOOBBDetector(OfflineDetector):
    ...
    def _resolve_device(self, device: str) -> torch.device:
        ...
        if requested.startswith("cuda"):
            ...
            self.logger.warning("YOLO OBB: CUDA를 요청했지만 쓸 수 없어 CPU로 폴백합니다")
        elif requested.startswith("mps"):
            if getattr(torch.backends, "mps", None) and torch.backends.mps.is_available():
                return torch.device("mps")
            self.logger.warning("YOLO OBB: MPS를 요청했지만 쓸 수 없어 CPU로 폴백합니다")
    ...
    async def _load(self, device: str):
        ...
        self.logger.info(f"YOLO OBB: {self.torch_device.type.upper()} 모드 로드 성공")
    ...
    def _extract_prediction_arrays(
        ...
        if not np.all(valid_cls_mask):
            ...
            self.logger.info(f"YOLO OBB잘못된 클래스를 걸러냈습니다: 제거 {drop_count} 개 박스")
    ...
    def _rearrange_detect_unified(
        ...
        if image is None or image.size == 0:
            self.logger.error("YOLO OBB: 입력 이미지가 유효하지 않습니다")
            return self._empty_results()

        h, w = image.shape[:2]
        if h == 0 or w == 0:
            self.logger.error(f"YOLO OBB: 이미지 크기가 0입니다: {h}x{w}")
            return self._empty_results()

        if rearrange_plan is None:
            rearrange_plan = build_det_rearrange_plan(image, tgt_size=self.input_size)
        if rearrange_plan is None:
            self.logger.warning("YOLO OBB통일 분할: 현재 이미지가 분할 조건에 맞지 않습니다")
            return self._empty_results()
        ...
        for ii, patch in enumerate(patch_array):
            if np.all(patch == 0):
                self.logger.debug(f"YOLO OBB patch {ii}: 패딩 패치를 건너뜁니다")
                continue

            if patch.size == 0 or patch.shape[0] == 0 or patch.shape[1] == 0:
                self.logger.warning(f"YOLO OBB patch {ii}: 잘못된 패치를 건너뜁니다, shape={patch.shape}")
                continue
            ...
            try:
                ...
                self.logger.error(f"YOLO OBB patch {ii} 추론 실패: {e}")
            ...
            if verbose:
                self.logger.debug(f"YOLO OBB patch {ii}: 감지 {len(boxes)} 개 박스")
                try:
                    import logging
                    ...
                    self.logger.error(f"YOLO 디버그 이미지 저장 실패: {e}")
        ...
        self.logger.info(f"YOLO OBB통일 분할 검출 완료: 병합·중복 제거 후 {len(boxes_corners)} 개 박스")
    ...
    async def _infer(
        ...
        """
        if image is None:
            self.logger.error("YOLO OBB: 받은 이미지가 None입니다")
            return [], None, None

        if not isinstance(image, np.ndarray):
            self.logger.error(f"YOLO OBB: 받은 값이 numpy 배열이 아닙니다. 유형: {type(image)}")
            return [], None, None

        if image.size == 0:
            self.logger.error("YOLO OBB: 받은 이미지 크기가 0입니다")
            return [], None, None

        if len(image.shape) < 2:
            self.logger.error(f"YOLO OBB: 이미지 차원이 부족합니다: {image.shape}")
            return [], None, None

        if image.shape[0] == 0 or image.shape[1] == 0:
            self.logger.error(f"YOLO OBB: 이미지 크기가 0입니다: {image.shape}")
            return [], None, None
        ...
        if rearrange_plan is not None:
            self.logger.info("YOLO OBB: 긴 이미지를 감지해 통일 분할을 사용합니다")
            ...
            try:
                ...
                self.logger.error(f"YOLO OBB추론 실패: {e}")
                self.logger.error(f"입력 이미지 shape: {image.shape}, dtype: {image.dtype}")
                self.logger.error(f"현재 device: {self.device}")
                raise
        ...
        self.logger.info(f"YOLO OBB감지 {len(textlines)} 개 텍스트 박스")
