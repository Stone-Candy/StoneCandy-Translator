...
class DefaultDetector(OfflineDetector):
    ...
    async def _infer(self, image: np.ndarray, detect_size: int, text_threshold: float, box_threshold: float,
        ...
        if image is None or image.size == 0:
            self.logger.error("입력 이미지가 비어 있거나 유효하지 않습니다")
            return [], np.zeros((100, 100), dtype=np.uint8), None
        
        if len(image.shape) < 2:
            self.logger.error(f"입력 이미지 차원이 잘못되었습니다: {image.shape}")
            return [], np.zeros((100, 100), dtype=np.uint8), None
        ...
        if verbose:
            try:
                ...
                self.logger.info(f'[DEBUG] mask값 분포: min={mask.min():.3f}, max={mask.max():.3f}, mean={mask.mean():.3f}')
                self.logger.info(f'[DEBUG] mask 중간값(0.1-0.9) 비율: {mid_values_ratio_mask:.2f}%')
                
                # 检查db的分布（用于对比）
                db_slice = db[0, 0, :, :]
                mid_values_ratio_db = np.sum((db_slice > 0.1) & (db_slice < 0.9)) / db_slice.size * 100
                self.logger.info(f'[DEBUG] db값 분포: min={db_slice.min():.3f}, max={db_slice.max():.3f}, mean={db_slice.mean():.3f}')
                self.logger.info(f'[DEBUG] db 중간값(0.1-0.9) 비율: {mid_values_ratio_db:.2f}%')
