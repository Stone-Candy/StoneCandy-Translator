...
- `manga_translator_work/json/<image>_translations.json`
- `manga_translator_work/script/0000 script.txt` or `combined script.txt`

**Manual translation flow**:

1. Open `0000 script.txt` or `combined script.txt`
...
- Highest: script TXT (`script/0000 script.txt` or `combined script.txt`)
- Then: `_translated.txt`
- Otherwise: JSON translation fields

Qt UI tip:

- `Tip: After exporting, manually translate 0000 script.txt in manga_translator_work/script/, then use 'Import Translation and Render' mode`
...
- Deletes the matching page `0000 script.txt` after a successful write-back

Qt UI tip:

- `Tip: Requires existing JSON data. The app reads original text from JSON, translates it, writes results back to JSON, and deletes 0000 script.txt after success`
...
- Check whether script TXT or `_translated.txt` exists
...
- script TXT from `manga_translator_work/script/`
...
- `Tip: Will read TXT files from manga_translator_work/script/ or translations/ and render (prioritize script.txt)`
...
- `script/` and `translations/` support manual translation workflows
