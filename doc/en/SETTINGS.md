...
- **`Translate JSON Only` (`translate_json_only`)**: extract original text from existing JSON, run translation, and write the result back into the JSON.
  ...
  - After the JSON is written successfully, the matching page `0000 script.txt` file is deleted
...
- **`Launch Photoshop Immediately After Generating PSD Script` (`psd_launch_photoshop`)**: after generating the `.jsx` script, immediately launch Photoshop to produce the PSD.
  - Current UI location: `Settings` -> `General` -> `Launch Photoshop Immediately After Generating PSD Script`
  - Default: off (script only; Photoshop is not launched)
...
- **`Font` (`font_family`)**: Qt font family used for typesetting. The list includes OS fonts and files in the project `fonts` directory.
  - Internal config key: `render.font_family`
  - Region JSON stores `font_family` instead of `font_path`
...
- **`Disable System Fonts` (`disable_system_fonts`)**: when enabled, the font list only shows project `fonts` directory files.
