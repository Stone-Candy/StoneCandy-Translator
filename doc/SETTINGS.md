...
- **仅翻译（JSON）(translate_json_only)**：从已有 JSON 提取原文并执行翻译，然后回写 JSON
  ...
  - 成功回写 JSON 后，会删除对应的页面 `0000 script.txt`
...
- **生成 PSD 脚本后立即启动 Photoshop (psd_launch_photoshop)**：生成 `.jsx` 脚本后立即启动 Photoshop 导出 PSD
  - 默认：关闭（只生成脚本，不启动 Photoshop）
...
- **字体 (font_family)**：按字体家族选择嵌字字体（系统字体 + 项目 `fonts` 目录）
  - JSON 区域数据保存 `font_family`，不再保存 `font_path`
  - 可以在 `fonts` 目录下添加新的字体文件（`.ttf`、`.otf`、`.ttc` 格式）
  - 打开下拉菜单时会刷新列表

- **禁止使用系统字体库 (disable_system_fonts)**：开启后字体列表只显示项目 `fonts` 目录中的字体
