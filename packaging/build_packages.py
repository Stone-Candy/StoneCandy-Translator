...
class Builder:
    ...
    def bundle_settings_tab_layout(self, dist_dir):
        """将设置页分类布局 JSON 写入打包产物，使菜单与开发版一致。"""
        src = Path("desktop_qt_ui") / "ui" / "main_page" / "settings_tab_layout.json"
        dst_dir = Path(dist_dir) / "_internal" / "desktop_qt_ui" / "ui" / "main_page"
        if not src.exists():
            print(f"Warning: settings tab layout not found: {src}")
            return
        dst_dir.mkdir(parents=True, exist_ok=True)
        dst = dst_dir / src.name
        shutil.copy2(src, dst)
        print(f"Bundled settings tab layout: {dst}")
    ...
    def build_executables(self, version_type):
        ...
        self.bundle_settings_tab_layout(dist_dir)
