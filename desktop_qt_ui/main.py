...
from utils.resource_helper import (
    ...
    iter_existing_resource_paths,
    load_icon_from_resources,
)
...
def global_exception_handler(exc_type, exc_value, exc_traceback):
    ...
    logging.critical(f"Uncaught exception crashed the application:\n{error_msg}")
...
def _apply_macos_native_app_icon(icon_path: str):
    ...
    try:
        ...
        if not image:
            logging.warning(f"Failed to load macOS native app icon: {icon_path}")
            return False

        NSApplication.sharedApplication().setApplicationIconImage_(image)
        logging.info(f"macOS native app icon set: {icon_path}")
        return True
    except ImportError:
        logging.info("PyObjC/AppKit is not installed; skipping macOS Dock icon")
    except Exception:
        logging.exception("Failed to set macOS native app icon")
    return False
...
def main():
    ...
    try:
        ...
        logging.warning(f"Failed to create config files: {e}")
    ...
    logging.info("Crash capture (faulthandler) enabled; crash details will be written to this log file")
    ...
    app.setApplicationName("StoneCandy Translator")
    app.setOrganizationName("StoneCandy Translator")
    ...
    if app_icon and not app_icon.isNull():
        ...
        logging.info(f"UI icon loaded: {icon_source}")
    else:
        logging.warning("Failed to load UI icon: no usable icon.ico/icon.png/icon.icns")
    ...
    if sys.platform == 'darwin':
        ...
        if native_macos_icon_path:
            ...
            logging.warning("macOS native app icon not found: doc/images/icon.icns")
    ...
    if sys.platform == 'win32':
        ...
        if not native_windows_icon_path:
            logging.warning("Windows native window icon not found: doc/images/icon.ico")
    ...
    if sys.platform == 'win32' and native_windows_icon_path:
        apply_windows_native_window_icon(main_window, native_windows_icon_path)
        ...
        try:
            ...
            if sys.platform == 'win32':
                try:
                    ...
                    logging.debug(f"Failed to bring window to foreground on Windows: {exc}")
        except Exception as exc:
    ...
    #         show_startup_notice_if_enabled(main_window)
    #     except Exception:
    #         logging.exception("Failed to show startup notice")
    #
    # QTimer.singleShot(400, show_startup_notice)
    ...
    try:
        ...
        logging.error(f"Error while shutting down services: {e}", exc_info=True)
    ...
    try:
        ...
        print(f"Error while closing log handlers: {e}", file=sys.stderr)
