...
class RenderSettings(BaseModel):
    ...
    char_width: Optional[float] = 1.0  # 글자 가로폭 배율，기본 1.0
...
class CliSettings(BaseModel):
    ...
    context_size: int = 3
    format: str = "不指定"
    overwrite: str = "ask"  # ask / always / skip
    skip_no_text: bool = False
    save_text: bool = True
    load_text: bool = False
    rerender_only: bool = False  # JSON + inpaint data로 이미지만 다시 렌더링
    ...
    psd_launch_photoshop: bool = False  # 스크립트 생성 후 Photoshop 실행 (기본 끔 = 스크립트만 생성)
    replace_translation: bool = False  # 替换翻译模式：将一张图的翻译应用到另一张生肉图上
    combine_txt: bool = True  # script를 하나의 파일로 출력
    backup_combined_txt: bool = True  # 하나의 파일로 출력할 때 combined_script_BACKUP.txt 생성
    write_txt_help: bool = True  # TXT 내보내기 폴더에 도움말 TXT 생성

    @model_validator(mode="before")
    @classmethod
    def _normalize_overwrite(cls, data: Any):
        if not isinstance(data, dict) or "overwrite" not in data:
            return data
...
class AppSection(BaseModel):
    ...
    show_colorize_upscale_inpaint_modes: bool = False  # 작업 목록에 색상화/업스케일/클리닝 모드 표시
    show_novelai_mode: bool = False  # 작업 목록에 NovelAI 모드 표시
