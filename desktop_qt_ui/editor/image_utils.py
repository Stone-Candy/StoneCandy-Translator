...
def build_mask_display_frame(
    ...
    *,
    ...
    color: tuple[int, int, int] = (255, 0, 38),
    alpha: int = 128,
...
def unbake_matching_overlay(
    ...
    *,
    ...
    """덧칠 RGB와 인페인트가 거의 같은 칸은 예전에 구워진 것으로 보고 원본을 되돌린다.

    덧칠 색과 다른 인페인트는 그대로 둔다. 매칭이 없으면 입력을 그대로 반환한다.
    """
    if inpainted is None or overlay is None or original is None:
        return inpainted

    overlay_arr = np.asarray(overlay)
    if overlay_arr.ndim != 3 or overlay_arr.shape[2] < 4:
        return inpainted
    opaque = overlay_arr[..., 3] > 0
    if not np.any(opaque):
        return inpainted

    inpainted_rgb = image_like_to_rgb_array(inpainted, copy=True)
    original_rgb = image_like_to_rgb_array(original, copy=False)
    if inpainted_rgb is None or original_rgb is None:
        return inpainted

    height, width = inpainted_rgb.shape[:2]
    if overlay_arr.shape[:2] != (height, width):
        overlay_arr = cv2.resize(overlay_arr, (width, height), interpolation=cv2.INTER_NEAREST)
        opaque = overlay_arr[..., 3] > 0
    if original_rgb.shape[:2] != (height, width):
        original_rgb = cv2.resize(original_rgb, (width, height), interpolation=cv2.INTER_NEAREST)

    overlay_rgb = overlay_arr[..., :3].astype(np.int16, copy=False)
    diff = np.max(np.abs(inpainted_rgb.astype(np.int16, copy=False) - overlay_rgb), axis=2)
    baked = opaque & (diff <= int(match_tolerance))
    if not np.any(baked):
        return inpainted

    inpainted_rgb[baked] = original_rgb[baked]
