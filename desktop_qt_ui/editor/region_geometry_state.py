...
class RegionGeometryState:
    ...
    def visual_center_world(self) -> Tuple[float, float]:
        """World position of the visible box center (white frame / render box).

        Dragging a layer moves white_frame_rect_local and leaves source `center`
        at the original detection. Placement (paste under cursor) must use this.
        """
        wf = self.white_frame_local
        if wf is not None:
            try:
                left, top, right, bottom = (float(v) for v in wf[:4])
...
def _offset_xy(point, dx: float, dy: float):
    try:
        x = float(point[0]) + dx
        y = float(point[1]) + dy
...
def _offset_point_list(points, dx: float, dy: float):
    if not isinstance(points, (list, tuple)):
        return points
    return [_offset_xy(p, dx, dy) for p in points]


def region_visual_world_points(region_data: dict) -> List[Tuple[float, float]]:
    """Visible-box corners (or fallback outline) in image pixels."""
    if not isinstance(region_data, dict):
        return []

    if region_data.get("distortMode"):
        from editor.desktop_ui_geometry import normalize_distort_quad

        pts = normalize_distort_quad(region_data.get("distort_quad"))
        if pts:
            return [(float(p[0]), float(p[1])) for p in pts]

    geo = RegionGeometryState.from_region_data(region_data)
    wf = geo.white_frame_local
    if wf is not None:
        try:
            left, top, right, bottom = (float(v) for v in wf[:4])
            return [
                geo.local_to_world(left, top),
                geo.local_to_world(right, top),
                geo.local_to_world(right, bottom),
                geo.local_to_world(left, bottom),
            ]
        except (TypeError, ValueError):
            pass

    xyxy = region_data.get("xyxy")
    if isinstance(xyxy, (list, tuple, np.ndarray)) and len(xyxy) >= 4:
        try:
...
def region_visual_aabb_world(region_data: dict) -> Tuple[float, float, float, float]:
    """Axis-aligned bounds of the visible box in image pixels."""
    pts = region_visual_world_points(region_data)
    if not pts:
        cx, cy = region_visual_center_world(region_data)
        return (cx, cy, cx, cy)
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
...
def region_visual_center_world(region_data: dict) -> Tuple[float, float]:
    """Visible layer center in image pixels.

    4-point distort stores the on-screen quad in world space (`distort_quad`).
    Moving that quad does not update source `center` or the white frame, so
    paste must aim at the quad, not the white-frame center.
    """
    if isinstance(region_data, dict) and region_data.get("distortMode"):
        from editor.desktop_ui_geometry import normalize_distort_quad
    ...
    geo = RegionGeometryState.from_region_data(region_data if isinstance(region_data, dict) else {})
    return geo.visual_center_world()


def offset_region_world_coords(region_data: dict, dx: float, dy: float) -> None:
    """Translate world-space points. Local white-frame/render rects stay as-is."""
    if not isinstance(region_data, dict):
        return
    ...
    if isinstance(center, (list, tuple, np.ndarray)) and len(center) >= 2:
        region_data["center"] = [float(center[0]) + dx, float(center[1]) + dy]

    for key in ("lines", "polygons"):
        seq = region_data.get(key)
        if seq:
            region_data[key] = [_offset_point_list(poly, dx, dy) for poly in seq]
    ...
    if region_data.get("distort_quad") is not None:
        from editor.desktop_ui_geometry import normalize_distort_quad

        quad = normalize_distort_quad(region_data.get("distort_quad"))
        if quad:
            region_data["distort_quad"] = _offset_point_list(quad, dx, dy)
