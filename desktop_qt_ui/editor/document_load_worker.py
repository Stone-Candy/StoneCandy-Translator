...
from .image_utils import unbake_matching_overlay
...
class DocumentLoadWorker:
    ...
    def _load_snapshot(self) -> DocumentSnapshot:
        ...
        # 결과 폴더의 이미지를 translation_map으로 연 경우, 내보내기는 그 폴더로 돌아가야 한다.
        # source_path는 원본(예: 만화 원본/...)으로 바뀌므로 dirname(source)는 쓰면 안 된다.
        preferred_export_dir = None
        opened_path = os.path.normpath(self.image_path)
        mapped_source = self.service.find_source_from_translation_map(opened_path)
        if mapped_source:
            preferred_export_dir = os.path.dirname(opened_path)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.AUX_WORKERS) as executor:
            ...
            inpainted_image = unbake_matching_overlay(
                inpainted_image,
                paint_overlay_image,
                image,
            )
        ...
        return DocumentSnapshot(
            ...
            preferred_export_dir=preferred_export_dir,
        )
