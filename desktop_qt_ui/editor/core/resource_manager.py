...
class ResourceManager:
    ...
    def load_image(self, image_path: str) -> ImageResource:
        ...
        if image_path in self._image_cache:
            ...
            # 缓存命中也要刷新 LRU，否则当前页会因旧 load_time 被预读淘汰并 close。
            if hasattr(resource, "touch"):
                resource.touch()
            else:
                resource.load_time = time.time()
            # 若资源已被错误 release（image=None / closed），重新打开。
            if resource.image is None or not self._is_live_pil_image(resource.image):
                self.logger.warning(
                    "Cached image resource is missing or closed, reloading: %s",
                    image_path,
                )
    ...
    @staticmethod
    def _is_live_pil_image(image: Any) -> bool:
        """PIL Image 是否仍可 copy（未被 close）。"""
    ...
    def _protected_cache_paths(self) -> set[str]:
        """淘汰时禁止 close 的路径（当前编辑底图）。"""
        protected: set[str] = set()
        if self._current_image is not None and getattr(self._current_image, "path", None):
            protected.add(self._current_image.path)
        return protected

    def _evict_oldest_cache_entry(self, extra_protect: Optional[set[str]] = None) -> bool:
        """淘汰一张非受保护的最旧缓存图。成功淘汰返回 True。"""
        if not self._image_cache:
            return False

        protected = self._protected_cache_paths()
        if extra_protect:
            protected |= extra_protect

        candidates = [
            (path, resource)
            for path, resource in self._image_cache.items()
            if path not in protected
        ]
        if not candidates:
            self.logger.debug(
                "Image cache full but only protected images remain; skip eviction (size=%s limit=%s)",
                len(self._image_cache),
                self._cache_limit,
            )
            return False

        oldest_path, old_resource = min(candidates, key=lambda item: item[1].load_time)
        self._image_cache.pop(oldest_path, None)
        # 二次确认：绝不 close 当前底图（Session 与 ImageResource 共享同一 PIL）
        if oldest_path in protected or (
            self._current_image is not None and old_resource is self._current_image
        ):
            self.logger.warning("Refused to release protected image resource: %s", oldest_path)
            self._image_cache[oldest_path] = old_resource
            return False
    ...
    def _add_to_cache(self, path: str, resource: ImageResource) -> None:
        ...
        if path in self._image_cache:
            existing = self._image_cache.get(path)
            if existing is not None and existing is not resource:
                # 同一 path 换新 resource 时，旧对象若不是 current 才可 release
                if self._current_image is not existing:
                    existing.release()
        ...
        # 缓存已满时淘汰最旧的【非当前】图。
        # 旧逻辑会按 load_time 淘汰，缓存命中不刷新 load_time 时
        # 当前编辑页反而最旧，被相邻页 prefetch close → 导出 Operation on closed image。
        while len(self._image_cache) >= self._cache_limit:
            if not self._evict_oldest_cache_entry(extra_protect={path}):
                break
        self._image_cache[path] = resource
    ...
    def release_image_from_cache(self, path: str) -> bool:
        ...
        if path in self._protected_cache_paths():
            self.logger.warning("Refused to release current/protected image from cache: %s", path)
            return False
        if path in self._image_cache:
            resource = self._image_cache.pop(path)
            if self._current_image is not None and resource is self._current_image:
                # 二次保护：即使 path 判断漏网也不 close 当前底图
                self._image_cache[path] = resource
                self.logger.warning("Refused to release current image resource from cache: %s", path)
                return False
            resource.release()
    ...
    def clear_image_cache(self) -> None:
        ...
        当前编辑底图（_current_image）不会 close——Session 可能仍持有同一 PIL。
        仅从 dict 中移除并释放非当前图。
        """
        current = self._current_image
        current_path = current.path if current is not None else None
        for path, resource in list(self._image_cache.items()):
            if current is not None and (resource is current or path == current_path):
                continue
            self._image_cache.pop(path, None)
            resource.release()
        # 保留当前图在缓存中，避免 Session 持有已 close 的僵尸引用
        if current is not None and current_path:
            self._image_cache[current_path] = current
        else:
            # 无当前文档时才允许彻底清空
            for resource in list(self._image_cache.values()):
                resource.release()
            self._image_cache.clear()
        _release_gpu_memory()
        self.logger.info("Cleared image cache (current protected=%s)", bool(current_path))
    ...
    def release_memory_after_export(self) -> None:
        """导出后提示系统回收工作集，但不丢弃当前文档状态。

        `_temp_cache` holds the live inpainted image, paint overlay, and
        incremental-inpaint snapshots for the open document. Clearing it
        here used to leave the overlay pixmap stale while the next brush
        stroke inpainted onto the original.
        """
