...
class EditorControllerDocumentService:
    ...
    def __init__(self, controller: "EditorController"):
        ...
        self._unsaved_prompt: Optional[QMessageBox] = None
    ...
    def _t(self, key: str) -> str:
        """i18n 번역 헬퍼 (editor_toolbar.py 등과 동일한 패턴)"""
    ...
    def prompt_unsaved_edits(self) -> str:
        """Ask what to do with unsaved edits.

        Returns:
            "clean": nothing to save
            "cancel": stay on the current image
            "export": persist first
            "discard": drop in-memory edits
        """
        if not self.controller.export_service.has_changes_since_last_export():
            return "clean"

        # exec() still runs a nested event loop. Without a parent / app-modal
        # flag the main window stays clickable, and each Back / file click
        # stacked another copy of this prompt.
        if self._unsaved_prompt is not None:
            self._unsaved_prompt.raise_()
            self._unsaved_prompt.activateWindow()
            return "cancel"
        ...
        msg_box.setWindowTitle(self._t("Unsaved Edits"))
        msg_box.setText(self._t("Current image has unsaved edits"))
        msg_box.setInformativeText(self._t("Exporting the image will also save the JSON."))

        export_btn = msg_box.addButton(self._t("Export Image"), QMessageBox.ButtonRole.YesRole)
        cancel_btn = msg_box.addButton(self._t("Cancel"), QMessageBox.ButtonRole.RejectRole)
        msg_box.addButton(self._t("Don't Save"), QMessageBox.ButtonRole.NoRole)

        msg_box.setDefaultButton(cancel_btn)
        apply_message_box_style(msg_box)
        msg_box.setWindowModality(Qt.WindowModality.ApplicationModal)

        self._unsaved_prompt = msg_box
        try:
            msg_box.exec()
        finally:
            self._unsaved_prompt = None
    ...
    def discard_unsaved_edits(self) -> None:
        """Reload the current image from disk so Don't Save actually drops edits."""
        source_path = self.model.get_source_image_path()
        if source_path:
            self.do_load_image(source_path)
            return
        self.clear_editor_state()

    def reset_session_after_leave_to_main(self) -> None:
        """Drop the leftover editor session so the next entry is a fresh load.

        Leaving used to keep the document in memory. Background font-fit / inpaint
        could then mark it dirty again, and re-entry showed "unsaved edits"
        even after the user had already saved or discarded.
        """
        self.clear_editor_state(release_image_cache=True)

    def request_leave_to_main(self) -> None:
        action = self.prompt_unsaved_edits()
        if action == "cancel":
            return
        if action == "export":
            export_future = self.controller.export_image()
            if export_future is None:
                self.logger.warning("Export request was not scheduled; aborted leave to main.")
                return
            export_future.add_done_callback(self._continue_leave_after_export)
            return
        # Do not reload here. A background reload after leave can dirty the
        # leftover session again before the next editor entry.
        self.controller.leave_to_main_allowed.emit()
    ...
    def _continue_leave_after_export(self, future) -> None:
        try:
            result = future.result()
        except Exception as e:
            self.logger.error("Leave to main skipped because export task failed: %s", e, exc_info=True)
            return
        ...
        self.logger.info(
            "Leave to main skipped because export did not complete successfully: %s",
            result,
        )
    ...
    def do_load_image(self, image_path: str) -> None:
        ...
        if toast_manager is not None:
            self.controller._loading_toast = toast_manager.show_info(self._t("Loading..."), duration=0)
    ...
    def apply_loaded_data_to_model(self, snapshot: DocumentSnapshot) -> None:
        ...
        if self.controller._show_original_only:
            # 원본 보기 토글(Z)이 켜져 있으면 페이지가 바뀌어도 원본만 보이는 상태를 유지
            self.controller.set_original_image_alpha(100)
        elif not self.controller._user_adjusted_alpha:
    ...
    def handle_load_error(self, error_msg: str) -> None:
        ...
        toast_manager = self.controller.get_toast_manager()
        if toast_manager is not None:
            toast_manager.show_error(f"{self._t('Load Failed')}: {error_msg}")
