...
from services import get_i18n_manager, get_render_parameter_service
...
class EditorControllerExportService:
    ...
    def _t(self, key: str) -> str:
        """i18n 번역 헬퍼 (editor_toolbar.py / graphics_view_input.py와 동일한 패턴)"""
        ...
        try:
            ...
            if image is None:
                ...
                if toast_manager is not None:
                    toast_manager.show_error(self._t("Export Failed: Missing Image Data"))
                return
            ...
            if mask is None and regions:
                ...
                toast_manager = self.controller.get_toast_manager()
                if toast_manager is not None:
                    toast_manager.show_error(self._t("Export Failed: No Mask Data Available"))
                return None

            self.controller._export_toast = None
            toast_manager = self.controller.get_toast_manager()
            if toast_manager is not None:
                # A finite fallback prevents an orphaned progress toast from remaining
                # onscreen when overlapping export requests replace _export_toast.
                self.controller._export_toast = toast_manager.show_info(self._t("Exporting..."), duration=15_000)
            ...
            # 작업용 inpainted PNG에는 덧칠을 굽지 않는다.
            # 결과 이미지용으로만 합성본을 백엔드에 넘긴다.
            persist_inpainted = self.model.get_inpainted_image()
            persist_snapshot = self.controller._snapshot_image_for_export(
                persist_inpainted,
                "inpainted image",
            )
            render_base = persist_inpainted
            if render_base is None and overlay_snapshot is not None and source_path:
                render_base = self._load_existing_inpainted_for_compose(source_path)
            if overlay_snapshot is not None and render_base is not None:
                composed = self.compose_image_with_overlay(render_base, overlay_snapshot)
                if composed is not None and composed is not render_base:
                    render_base = composed
            render_snapshot = self.controller._snapshot_image_for_export(
                render_base,
                "render inpainted image",
            )
            regions_snapshot = copy.deepcopy(regions)
            mask_snapshot = None if mask is None else np.array(mask, copy=True)

            # 乐观更新：提交异步任务前先打快照，避免 Ctrl+S 后立刻切图弹"未保存的编辑"。
            # 失败时 success_callback 不会再次刷新快照，用户重新 Ctrl+S 即可。
            ...
            # 先关闭"正在导出"Toast，避免 duration=0 导致它永久卡住
            if hasattr(self.controller, '_export_toast') and self.controller._export_toast:
                try:
                    self.controller._export_toast.close()
                    self.controller._export_toast = None
                except Exception as close_err:
                    self.logger.warning(f"Failed to close export toast: {close_err}")
            toast_manager = self.controller.get_toast_manager()
            if toast_manager is not None:
                toast_manager.show_error(self._t("Export Failed"))
            return None
    ...
    def save_current_inpainted_image(
        ...
        try:
            ...
            try:
                ...
                from manga_translator.utils.stealth_pngcomp import resolve_stealth_pngcomp_payload

                save_pil_image(
                    save_image,
                    inpainted_path,
                    stealth_pngcomp=resolve_stealth_pngcomp_payload(source_path),
                )
                ...
                self.logger.info(f"복구 이미지를 갱신했습니다: {inpainted_path}")
            finally:
                try:
                    save_image.close()
                except Exception:
            ...
            self.logger.warning(f"inpainted 이미지 갱신 실패: {e}")
    ...
    def save_paint_overlay_image(
        ...
        try:
            ...
            try:
                ...
                pil_overlay.close()
            self.logger.info(f"컬러 브러시 레이어를 갱신했습니다: {overlay_path}")
            return overlay_path
        except Exception as e:
            self.logger.warning(f"컬러 브러시 레이어 저장 실패: {e}")
            return None
    ...
    def _load_existing_inpainted_for_compose(self, source_path: str):
        ...
        try:
            ...
            self.logger.warning(f"디스크 inpainted를 캔버스 배경으로 로드하지 못했습니다: {e}")
            return None
    ...
    def persist_editor_state_for_export(
        ...
        # _save_regions_data_with_path는 해당 이미지 키의 JSON 엔트리를 통째로 새로 쓰기 때문에,
        # 원래 배치 번역 단계에서 기록해 둔 last_export_dir 필드가 사라진다.
        # 그 결과 다음 편집기 저장 시 _read_saved_export_dir()가 이를 찾지 못해
        # 출력 폴더 루트(config.app.last_output_path)로 fallback하며 하위 폴더 구조를 잃는다.
        # 여기서 방금 실제로 사용한 output_dir을 다시 기록해 이 문제를 막는다.
        if output_dir:
            self._update_last_export_dir(json_path, source_path, output_dir)
    ...
    def _refresh_file_list_json_status(self) -> None:
        view = getattr(self.controller, "view", None)
        file_list = getattr(view, "file_list", None) if view is not None else None
        if file_list is None or not hasattr(file_list, "refresh_json_status"):
            return
        QMetaObject.invokeMethod(file_list, "refresh_json_status", Qt.ConnectionType.QueuedConnection)

    def _update_last_export_dir(self, json_path: str, source_path: str, output_dir: str) -> None:
        """방금 사용한 output_dir을 JSON의 last_export_dir 필드에 다시 기록한다."""
        ...
        try:
            ...
            if not isinstance(image_data, dict):
                # _save_regions_data_with_path가 사용한 키(절대경로)를 못 찾으면
                # 유일한 엔트리를 사용 (해당 JSON은 이미지 1개당 1개 파일 구조이므로 안전)
                if len(data) == 1:
                    image_key = next(iter(data.keys()))
                    image_data = data[image_key]
                else:
                    return

            if image_data.get("last_export_dir") != output_dir:
                image_data["last_export_dir"] = output_dir
                with open(json_path, "w", encoding="utf-8") as f:
                    _json.dump(data, f, indent=2, ensure_ascii=False)
        except Exception as e:
            self.logger.debug(f"Failed to update last_export_dir for {source_path}: {e}")
    ...
    def _find_export_dir_via_translation_map(
        self,
        source_path: Optional[str],
        search_roots: list[Optional[str]],
    ) -> Optional[str]:
        """translation_map.json 역조회로 원본→결과 폴더를 찾는다."""
        if not source_path:
            return None
        ...
        source_normcase = os.path.normcase(source_norm)
        seen: set[str] = set()
            ...
            try:
                for name in os.listdir(root_dir):
                    ...
                    if not os.path.isdir(child):
                        continue
                    child_key = os.path.normcase(os.path.normpath(child))
                    if child_key in seen:
                        continue
                    seen.add(child_key)
                    candidate_dirs.append(os.path.normpath(child))
                    # 출력 루트 바로 아래 챕터 폴더 정도만 본다 (과도한 스캔 방지)
        ...
        for directory in candidate_dirs:
            ...
            try:
                ...
                for translated_path, mapped_source in data.items():
                    if not isinstance(mapped_source, str):
                        continue
                    try:
                        if os.path.normcase(os.path.normpath(mapped_source)) == source_normcase:
                            return os.path.dirname(os.path.normpath(str(translated_path)))
                    except Exception:
                        continue
            except Exception as e:
    ...
    def _resolve_editor_output_dir(self, config, source_path: Optional[str]) -> str:
        """편집기 내보내기 폴더 결정.

        우선순위:
        1) 결과 이미지를 연 폴더 (preferred_export_dir / translation_map)
        2) JSON last_export_dir — 단, 출력 루트만 가리키고 더 구체적인 후보가 있으면 무시
        3) translation_map 역조회
        4) app.last_output_path
        5) 원본 이미지 디렉터리
        """
        last_output = None
        if hasattr(config, "app"):
            last_output = self._normalize_existing_dir(getattr(config.app, "last_output_path", None))

        preferred = None
        try:
            preferred = self._normalize_existing_dir(self.model.get_preferred_export_dir())
        except Exception:
            preferred = None

        mapped = self._normalize_existing_dir(
            self._find_export_dir_via_translation_map(
                source_path,
                [preferred, last_output],
            )
        )
        saved = self._normalize_existing_dir(self._read_saved_export_dir(source_path))

        specific_candidates = [d for d in (preferred, mapped) if d]
        last_output_normcase = os.path.normcase(last_output) if last_output else None

        # 과거에 잘못 기록된 last_export_dir(= 출력 루트)은 구체 후보가 있으면 버린다.
        if saved and last_output_normcase and os.path.normcase(saved) == last_output_normcase:
            if specific_candidates:
                saved = None

        ordered: list[str] = []
        for candidate in (preferred, mapped, saved, last_output):
            if not candidate:
                continue
        ...
        if source_path:
            source_dir = self._normalize_existing_dir(os.path.dirname(source_path))
            if source_dir and source_dir not in ordered:
                ordered.append(source_dir)
    ...
    def _build_output_path(self, config, source_path: Optional[str]) -> str:
        ...
        if save_to_source_dir and source_path:
            ...
            output_dir = self._resolve_editor_output_dir(config, source_path)
    ...
    def _build_enhanced_regions(self, regions: list[dict]) -> list[dict]:
        ...
        for index, region in enumerate(regions):
            ...
            repair_region_font_box_consistency(enhanced_region)
    ...
    async def async_export_with_desktop_ui_service(
        ...
        try:
            ...
            # 디스크에는 덧칠 없는 inpainted를 두고, 백엔드 렌더에만 합성본을 넘긴다.
            if render_inpainted_image is None:
            ...
            def success_callback(_message):
                ...
                success_message = f"{self._t('Export Successful')}\n{output_path}"
                if persisted_json_path:
                    success_message += f"\n{self._t('JSON Synced')}"
                self.controller._show_toast_signal.emit(success_message, 2500, True, output_path)

                if self.controller._is_same_source_image(self.model.get_source_image_path(), source_path):
                    self.save_export_snapshot()
                    self.resource_manager.release_memory_after_export()
                    self.controller.inpaint_service.sync_last_cache_from_model()
            ...
            def error_callback(message):
                ...
                self.controller._show_toast_signal.emit(f"{self._t('Export Failed')}：{message}", 2500, False, "")
            ...
            if not outcome["success"] and outcome["error"] is None:
                outcome["error"] = self._t("Export Did Not Return Success Status")
            return outcome
        except Exception as e:
            ...
            def _cleanup_and_alert():
                # 先关闭"正在导出"Toast（在主线程中安全关闭），避免 duration=0 导致它永久卡住
                # 이 경로는 _show_toast_signal을 거치지 않으므로 여기서 직접 닫아야 함
                if hasattr(self.controller, '_export_toast') and self.controller._export_toast:
                    try:
                        self.controller._export_toast.close()
                        self.controller._export_toast = None
                        ...
                        self.logger.warning(f"Failed to close export toast: {close_err}")
                QMessageBox.critical(None, self._t("Export Failed"), f"{self._t('An Unexpected Error Occurred During Export')}:\n{err_msg}")

            QTimer.singleShot(0, _cleanup_and_alert)
            return outcome
