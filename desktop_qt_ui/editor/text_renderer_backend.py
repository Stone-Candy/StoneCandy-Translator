...
from manga_translator.rendering import distort_source_font_size, text_render
...
def render_text_image_for_region(text_block: TextBlock, dst_points: np.ndarray, transform, render_params: dict, pure_zoom: float = 1.0, total_regions: int = 1):
    ...
    try:
        ...
        char_width_multiplier = render_params.get('char_width', 1.0)
        if char_width_multiplier is None:
            char_width_multiplier = getattr(text_block, 'char_width', 1.0) or 1.0
        italic = bool(render_params.get('italic', getattr(text_block, 'italic', False)))
        bold = bool(render_params.get('bold', getattr(text_block, 'bold', False)))
        ...
        if bool(getattr(text_block, "distortMode", False)) and not render_params.get("_cheap_distort"):
            font_size = distort_source_font_size(
                font_size,
                text_for_render,
                bool(text_block.horizontal),
                dst_points,
                line_spacing=float(line_spacing_multiplier or 1.0),
                letter_spacing=float(letter_spacing_multiplier or 1.0),
                char_width=float(char_width_multiplier or 1.0),
                target_lang=getattr(text_block, "target_lang", None),
            )
        ...
        if text_block.horizontal:
            rendered_surface = text_render.put_text_horizontal(
                ...
                char_width=char_width_multiplier,
                italic=italic,
                bold=bold,
