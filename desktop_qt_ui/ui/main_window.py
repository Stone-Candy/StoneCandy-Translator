...
from PyQt6.QtCore import QEvent, QLibraryInfo, QLocale, Qt, QTimer, QTranslator, pyqtSlot
...
class MainWindow(QMainWindow):
    ...
    def changeEvent(self, event):
        ...
        if self.isMaximized() and not was_maximized:
            QTimer.singleShot(0, self._fit_editor_image_after_maximize)

    def _fit_editor_image_after_maximize(self) -> None:
        """Fit the editor image once after the main window enters maximized state."""
        if not self.isMaximized() or self.stacked_widget.currentWidget() is not self.editor_view:
            return
        if self.editor_view is not None and self.editor_view.graphics_view is not None:
            self.editor_view.graphics_view.fit_to_window()

    def _update_window_title(self):
        """更新窗口标题，保持标题与版本号同步。"""
        self.setWindowTitle(format_app_title("StoneCandy Translator", self.app_version))
    ...
    def _ensure_editor_initialized(self):
        ...
        self.editor_view.back_to_main_requested.connect(self.editor_controller.request_leave_to_main)
        self.editor_controller.leave_to_main_allowed.connect(self._on_leave_to_main_allowed)

        # 에디터 file_list selection을 실제 로드된 이미지와 동기화
        # (메인 리스트에서 특정 페이지로 바로 진입했을 때 A/D 단축키가
        #  엉뚱한 페이지 기준으로 동작하던 문제 수정)
        self.editor_logic.image_loaded_in_editor.connect(self.editor_view.file_list.select_file_by_path)
        ...
        if hasattr(self.main_view, "batch_edit_panel"):
            self.main_view.batch_edit_panel.set_editor_context(
                current_image_getter=lambda: (
                    self.editor_controller.model.get_source_image_path()
                    if self.editor_controller
                    else None
                ),
                reload_callback=lambda path: (
                    self.editor_logic.load_image_into_editor(path)
                    if self.editor_logic
                    else None
                ),
            )
    ...
    def _check_system_theme_change(self):
        ...
        if current_system_theme != self.last_system_theme:
            self.logger.info(f"시스템 테마 변경: {self.last_system_theme} -> {current_system_theme}")
            ...
            if current_system_theme == "dark":
                ...
                if self.current_applied_theme != "dark":
                    ...
                    self.logger.info(f"사용자 환경설정 저장: {self.current_applied_theme}")
                # 切换到深色主题
                self._apply_theme("dark")
            else:
                # 系统切换到浅色
                ...
                self.logger.info(f"사용자 환경설정 복원: {user_pref}")
    ...
    def _connect_signals(self):
        ...
        self.app_logic.files_added.connect(lambda *_: self.main_view._refresh_batch_edit_catalog())
        self.app_logic.files_cleared.connect(self.main_view._refresh_batch_edit_catalog)
        self.app_logic.file_removed.connect(lambda *_: self.main_view._refresh_batch_edit_catalog())
        ...
        self.app_logic.files_added.connect(lambda *_: self.main_view.refresh_output_path_conflict_warning())
        self.app_logic.files_cleared.connect(self.main_view.refresh_output_path_conflict_warning)
        self.app_logic.file_removed.connect(lambda *_: self.main_view.refresh_output_path_conflict_warning())
        self.app_logic.output_path_updated.connect(lambda *_: self.main_view.refresh_output_path_conflict_warning())
        ...
        self.main_view.setting_changed.connect(lambda *_: self.main_view.refresh_output_path_conflict_warning())
        ...
        self.main_view.file_list.file_double_clicked.connect(self.on_file_selected_from_main_list)
    ...
    def _change_language(self, locale_code: str):
        ...
        if self.i18n and self.i18n.set_locale(locale_code):
            ...
            self.logger.info(f"언어가 전환되었습니다: {locale_code}")
    ...
    def on_task_completed(self, saved_files: list):
        ...
        try:
            if hasattr(self, "main_view"):
                self.main_view._refresh_batch_edit_catalog()
            if not saved_files:
                return
            ...
            self.logger.error(f"on_task_completed 예외 발생: {e}", exc_info=True)
            import traceback
            traceback.print_exc()
    ...
    def _should_prompt_open_results_in_editor(self) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"에디터 안내창 표시 여부 판단에 실패해 안내창을 표시하도록 폴백합니다: {e}")
            return True
    ...
    def _on_leave_to_main_allowed(self):
        """Leave the editor after the unsaved-edit prompt, then drop the session."""
        self.stacked_widget.setCurrentWidget(self.main_view)
        if self.editor_controller:
            self.editor_controller.reset_session_after_leave_to_main()
    ...
    def enter_editor_mode(self, file_to_load: str = None, files_to_load: list = None):
        ...
        try:
            ...
            self.logger.error(f"enter_editor_mode 예외 발생: {e}", exc_info=True)
            import traceback
            traceback.print_exc()

    def closeEvent(self, event):
        """处理窗口关闭事件"""
        if hasattr(self, "main_view") and hasattr(self.main_view, "batch_edit_panel"):
