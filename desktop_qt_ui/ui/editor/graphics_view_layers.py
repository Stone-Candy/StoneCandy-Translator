...
from .graphics_items import RegionTextItem, apply_smooth_pixmap_transform
...
class GraphicsViewLayersMixin:
    ...
    def on_image_changed(self, image):
        ...
        try:
            ...
            apply_smooth_pixmap_transform(self._image_item)
    ...
    def on_original_image_alpha_changed(self, alpha: float):
        ...
        # 페인트 오버레이(z=5)는 원본 이미지(z=2)보다 위에 그려지므로,
        # 원본 알파만 올려서는 절대 가려지지 않는다. alpha가 완전 불투명(원본만 보기)이면
        # 페인트 오버레이 아이템 자체를 숨기고, 그 외에는 다시 보이게 한다.
        paint_item = getattr(self.overlay_layers.paint_overlay, 'item', None)
        if paint_item is not None:
            paint_item.setVisible(alpha < 1.0)
        ...
        for item in self.scene.items():
            if isinstance(item, RegionTextItem):
                if mode == "full":
                    ...
                    # 숨기기 전에 자식/페인트 플래그를 먼저 끄고, 마지막에 item 자체를 숨긴다.
                    item.set_text_visible(False)
                    item.set_box_visible(False)
                    item.set_white_box_visible(False)
                    item.setVisible(False)

        # box_only/none 모드로 페이지를 로드하면 _update_text_visuals()가
        # 텍스트 pixmap 렌더를 건너뛴다. 이후 full/text_only로 모드만 바꾸면
        # 가시성 플래그만 켜지고 실제 텍스트 레이어는 비어 있게 되므로,
        # 텍스트를 보여야 하는 모드로 전환할 때 렌더를 보충한다.
        if mode in ("full", "text_only") and hasattr(self, "_update_text_visuals"):
            self._update_text_visuals()
            if mode == "full" and hasattr(self, "scene") and self.scene is not None:
                self.scene.update()

        # 숨김 계열(text_only/box_only/none): SmartViewportUpdate + CacheBackground 때문에
        # setVisible(False)/박스 끄기만으로는 이전 틀·텍스트 잔상이 남을 수 있어
        # 모드 전환 시에만 씬·뷰포트 전체를 한 번 갱신한다. full(1)은 위 부분 갱신만 사용.
        if mode in ("none", "text_only", "box_only"):
