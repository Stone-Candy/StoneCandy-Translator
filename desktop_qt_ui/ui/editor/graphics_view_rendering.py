...
class GraphicsViewRenderingMixin:
    ...
    def _infer_geometry_edit_kind(self, region_index: int, new_region_data: dict) -> str:
        ...
        if not self._values_equal(
            old_region_data.get("distortMode", False),
            new_region_data.get("distortMode", False),
        ) or not self._values_equal(
            old_region_data.get("distort_quad"),
            new_region_data.get("distort_quad"),
        ):
            return "distort"
    ...
    def _perform_single_item_update(self, index, edit_kind: str | None = None):
        try:
            ...
            # custom 흰 박스면 dst = 흰 박스(핸들과 동일). 없으면 calc_box_from_font.
            # render_center 는 snapshot 에서 흰 박스 중심으로 맞춘다.
    ...
    def _build_render_snapshot(self, index: int, region_data: dict, item: RegionTextItem | None) -> RegionRenderSnapshot:
        ...
        if item is not None and hasattr(item, "distort_state_patch"):
            data = dict(region_data or {})
            data.update(item.distort_state_patch())
    ...
    def preview_distort_region(self, index: int, distort_quad) -> None:
        from editor.desktop_ui_geometry import distort_quad_to_dst_points

        dst = distort_quad_to_dst_points(distort_quad)
        if dst is None:
            return
        self._recalculate_single_region_render_data(index, override_dst_points=dst)
        self._update_single_region_text_visual(index, cheap_distort=True)

    def _render_region_text_visual(self, index: int, use_cache: bool, cheap_distort: bool = False):
        if not (0 <= index < len(self._region_items)):
            return
        item = self._region_items[index]
        ...
        if cheap_distort:
            render_params["_cheap_distort"] = True
        ...
        if cached_result:
            ...
            use_render_center = snapshot.render_center
            if item.is_distort_mode():
                use_render_center = None
            item.update_text_pixmap(
                pixmap,
                pos,
                0,
                None,
                render_center=use_render_center,
            )
        else:
            clear_region_text(item)
    ...
    def _perform_render_update(self):
        ...
        try:
            ...
            # 새로 생성/갱신된 RegionTextItem은 기본값(full 모드)으로 초기화되므로,
            # 현재 표시 모드(text_only/none 등)를 다시 적용해야 페이지 전환 후에도
            # 모드가 유지된다. 모드 값 자체는 session에 남아 있지만, 값이 "바뀔 때만"
            # on_region_display_mode_changed()가 호출되던 기존 흐름에서는 같은 모드로
            # 페이지를 넘길 경우 새 아이템에 반영되지 않았다.
            self.on_region_display_mode_changed(self.model.get_region_display_mode())
    ...
    def _recalculate_single_region_render_data(self, index, override_dst_points=None):
        ...
        if region_dict.get("char_width") is not None:
            region_specific_params["char_width"] = region_dict.get("char_width")
        if region_dict.get("stroke_width") is not None:
            region_specific_params["stroke_width"] = region_dict.get("stroke_width")
        elif region_dict.get("text_stroke_width") is not None:
            region_specific_params["stroke_width"] = region_dict.get("text_stroke_width")

        prefer_white_frame = bool(region_dict.get("has_custom_white_frame", False))
        white_frame_local = (
            snapshot.white_frame_local
            if snapshot is not None and prefer_white_frame
            else None
        )
        try:
            self._dst_points_cache[index] = calculate_region_dst_points(
                ...
                prefer_white_frame=prefer_white_frame,
            )
        except Exception as e:
    ...
    def _update_single_region_text_visual(self, index, use_cache=False, cheap_distort: bool = False):
        try:
            self._render_region_text_visual(index, use_cache=use_cache, cheap_distort=cheap_distort)
        except (RuntimeError, AttributeError) as e:
    ...
    def recalculate_render_data(self):
        ...
        for i, text_block in enumerate(self._text_blocks_cache):
            ...
            try:
                ...
                if region_dict.get("char_width") is not None:
                    region_params["char_width"] = region_dict.get("char_width")
                if region_dict.get("stroke_width") is not None:
                    region_params["stroke_width"] = region_dict.get("stroke_width")
                elif region_dict.get("text_stroke_width") is not None:
                    region_params["stroke_width"] = region_dict.get("text_stroke_width")
                prefer_white_frame = bool(region_dict.get("has_custom_white_frame", False))
                white_frame_local = (
                    snapshot.white_frame_local
                    if snapshot is not None and prefer_white_frame
