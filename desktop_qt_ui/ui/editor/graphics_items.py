...
def apply_smooth_pixmap_transform(item: QGraphicsPixmapItem | None) -> None:
    """Enable bilinear filtering on a pixmap item.

    QGraphicsPixmapItem.paint() overwrites the view's SmoothPixmapTransform
    hint with its own transformationMode, which defaults to FastTransformation
    (nearest neighbor). That is why the editor looks harsher than a normal
    image viewer even when the view already requests smooth transforms.
    """
    if item is None:
        return
...
class TransparentPixmapItem(QGraphicsPixmapItem):
    ...
    def __init__(self, parent=None):
        ...
        apply_smooth_pixmap_transform(self)
...
class RegionTextItem(QGraphicsItemGroup):
    ...
    def __init__(self, region_data, region_index, geometry_callback, parent=None):
        ...
        self._distort_mode = False
        self._distort_quad = None
        self._apply_distort_state(self.region_data)
        ...
        self._drag_start_distort_quad = None
    ...
    def _apply_distort_state(self, region_data: dict):
        data = region_data if isinstance(region_data, dict) else {}
        self._distort_mode = bool(data.get("distortMode", False))
        self._distort_quad = normalize_distort_quad(data.get("distort_quad"))
        if self._distort_mode and self._distort_quad is None:
            self._distort_mode = False
        if self._distort_mode:
            self.setRotation(0)
        else:
            self.setRotation(self.rotation_angle)

    def is_distort_mode(self) -> bool:
        return bool(self._distort_mode and self._distort_quad)

    def distort_state_patch(self) -> dict:
        if not self.is_distort_mode():
            return {"distortMode": False, "distort_quad": None}
        return {
            "distortMode": True,
            "distort_quad": [list(p) for p in self._distort_quad],
        }

    def _distort_local_points(self) -> list:
        if not self.is_distort_mode():
            return []
        cx, cy = float(self.geo.center[0]), float(self.geo.center[1])
        return [
            QPointF(float(x) - cx, float(y) - cy)
            for x, y in self._distort_quad
        ]
    ...
    def set_white_box_visible(self, visible: bool):
        if self._show_white_box != visible:
            self._show_white_box = visible
            self.prepareGeometryChange()
            self._shape_path = None
            self.update()

    def _edit_chrome_visible(self) -> bool:
        """White-frame outline/handles: on in full/box_only, or for the selected item.

        text_only hides boxes globally, but the selected layer still needs
        handles so it can be moved/resized/rotated.
        """
        return bool(self._show_white_box) or self.isSelected()
    ...
    def shape(self) -> QPainterPath:
        try:
            ...
            path = QPainterPath()
            # Hit test uses the edit box even when text_only hides it.
            # Chrome visibility only controls drawing, not clickability.
            if self.is_distort_mode():
                local_pts = self._distort_local_points()
                if local_pts:
                    path.addPolygon(QPolygonF(local_pts))
                    if self.isSelected():
            ...
            elif self.geo.white_frame_local is not None:
    ...
    def paint(self, painter, option, widget=None):
        try:
            # 스포이드 콘텐츠 샘플: 핸들/선택 박스 등 에디터 크롬은 그리지 않음
            # (자식 text_item 렌더 글자는 그대로 합성됨)
            view = self._primary_view()
            if view is not None and getattr(view, "_eyedropper_hide_chrome", False):
                return

            painter.save()
            painter.setRenderHint(QPainter.RenderHint.Antialiasing)
            is_sel = bool(option.state & QStyle.StateFlag.State_Selected)
            chrome = self._edit_chrome_visible()

            # Unselected dashed frames only when the display mode shows boxes.
            # Selected items always get the frame so text_only can still be edited.
            if self.is_distort_mode() and (
                (self._show_white_box and self._polygons_visible) or is_sel
            ):
                self._draw_distort_box(painter, is_sel)
            elif self.geo.white_frame_local is not None and (
                (self._show_white_box and self._polygons_visible) or is_sel
            ):
                self._draw_white_box(painter, is_sel)

            if is_sel and chrome and self.is_distort_mode():
                self._draw_distort_handles(painter)
            elif is_sel and chrome and self.geo.white_frame_local is not None:
                ri = self._rotate_handle_info()
                hs = ri["handle_size"]
                pw = ri["pen_width"]
    ...
    def itemChange(self, change, value):
        try:
            if change in (
                ...
                # text_only: selection expands/shrinks the hit shape to include handles
                self.prepareGeometryChange()
                self._shape_path = None
    ...
    def _draw_distort_box(self, painter, is_selected: bool):
        local_pts = self._distort_local_points()
        if len(local_pts) != 4:
            return
        poly = QPolygonF(local_pts)
        if is_selected:
            painter.setPen(QPen(QColor(0, 255, 255), 4))
            painter.setBrush(QBrush(Qt.BrushStyle.NoBrush))
            painter.drawPolygon(poly)
    ...
    def _draw_distort_handles(self, painter):
        ...
        pw = mtx["pen_width"]
        half = hs / 2.0
        painter.setBrush(QBrush(QColor(255, 80, 180)))
        painter.setPen(QPen(QColor("black"), pw))
        for p in self._distort_local_points():
            painter.drawRect(int(p.x() - half), int(p.y() - half), int(hs), int(hs))
    ...
    def _get_handle_at(self, pos: QPointF):
        if self.is_distort_mode() and self._edit_chrome_visible():
            result = self._distort_handle_at(pos)
            if result[0] is not None:
                return result
            if self._point_in_distort_quad(pos):
                return "distort_move", (0, 0)
            return None, (-1, -1)
        ...
        if self._edit_chrome_visible() and self.geo.white_frame_local is not None:
    ...
    def _distort_handle_at(self, pos: QPointF):
        mtx = self._white_handle_metrics()
        hr_sq = mtx["hit_radius"] ** 2
        for i, p in enumerate(self._distort_local_points()):
            if (p.x() - pos.x()) ** 2 + (p.y() - pos.y()) ** 2 <= hr_sq:
                return "distort_corner", i
        return None, (-1, -1)

    def _point_in_distort_quad(self, pos: QPointF) -> bool:
        local_pts = self._distort_local_points()
        if len(local_pts) != 4:
            return False
        return QPolygonF(local_pts).containsPoint(pos, Qt.FillRule.OddEvenFill)
    ...
    def hoverMoveEvent(self, event: QGraphicsSceneMouseEvent):
        try:
            ...
            if self.isSelected():
                ...
                if handle == "distort_corner":
                    cursor_map["distort_corner"] = Qt.CursorShape.SizeAllCursor
                elif handle == "distort_move":
                    cursor_map["distort_move"] = Qt.CursorShape.SizeAllCursor
    ...
    def _save_drag_start_state(self, event, local_pos, handle, indices):
        ...
        if handle:
            ...
            elif handle in ("distort_corner", "distort_move"):
                self._drag_start_distort_quad = (
                    [list(p) for p in self._distort_quad] if self._distort_quad else None
                )
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        try:
            ...
            elif self._interaction_mode in ("distort_corner", "distort_move"):
                self._handle_distort_edit(event)
    ...
    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent):
        try:
            ...
            elif mode in ("distort_corner", "distort_move"):
                self._commit_distort(event)
    ...
    def _handle_distort_edit(self, event: QGraphicsSceneMouseEvent):
        if not self._drag_start_distort_quad:
            return
        start_quad = [list(p) for p in self._drag_start_distort_quad]
        scene_pos = event.scenePos()
        if self._interaction_mode == "distort_corner":
            idx = self._drag_handle_indices
            if not isinstance(idx, int) or not (0 <= idx < 4):
                return
            start_quad[idx] = [float(scene_pos.x()), float(scene_pos.y())]
        elif self._interaction_mode == "distort_move":
            ...
            return
        if self._interaction_mode == "distort_corner" and not is_convex_quad(start_quad):
            return

        old_rect = self.sceneBoundingRect() if self.scene() else None
        self.prepareGeometryChange()
        self._shape_path = None
        self._distort_mode = True
        self._distort_quad = start_quad
        self.update()
        self._invalidate_scene_rect(old_rect)
        self._preview_distort_render()

    def _preview_distort_render(self):
        view = self._primary_view()
        if view is None or not hasattr(view, "preview_distort_region"):
            return
        try:
            view.preview_distort_region(self.region_index, self._distort_quad)
        except Exception:
            logger.debug("[RegionTextItem] preview_distort_render failed", exc_info=True)

    def _commit_distort(self, event):
        if not self.is_distort_mode():
            super().mouseReleaseEvent(event)
            return
        new_data = copy.deepcopy(self.region_data)
        new_data.update(self.distort_state_patch())
        self._emit_region_update(event, new_data)
