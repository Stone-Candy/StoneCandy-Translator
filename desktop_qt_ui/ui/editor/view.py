...
class EditorView(QWidget):
    ...
    def __init__(self, app_logic: Any, model: EditorModel, controller: EditorController, logic: EditorLogic, parent=None):
        ...
        self.shortcut_list_button: QPushButton | None = None
        self._shortcut_list_dialog = None
        self.open_result_folder_button: QPushButton | None = None
        self.sync_file_dates_button: QPushButton | None = None
    ...
    def _create_left_panel(self) -> QWidget:
        ...
        translation_layout.setContentsMargins(8, 2, 2, 0)
        ...
        left_panel = QWidget()
        left_panel.setObjectName("editor_left_panel")
        left_layout = QVBoxLayout(left_panel)
        left_layout.setContentsMargins(0, 1, 0, 0)
        left_layout.setSpacing(0)
        left_layout.addWidget(self.left_tab_widget)
        left_panel.setMinimumWidth(self.left_tab_widget.minimumWidth())
        left_panel.setMaximumWidth(self.left_tab_widget.maximumWidth())
        return left_panel

    def _on_left_tab_changed(self, index: int):
        if index == 1 and self.region_list_view is not None:
    ...
    def refresh_tab_titles(self):
        ...
        self.left_tab_widget.setTabText(0, self._t("Property Editor"))
        self.left_tab_widget.setTabText(1, self._t("Editable Translation"))
    ...
    def refresh_ui_texts(self):
        ...
        if self.shortcut_list_button is not None:
            self.shortcut_list_button.setText(self._t("Shortcut List"))
        if self.open_result_folder_button is not None:
            self.open_result_folder_button.setText(self._t("Open Result Folder"))
        if self.sync_file_dates_button is not None:
            self.sync_file_dates_button.setText(self._t("Sync File Dates"))
            set_hover_hint(self.sync_file_dates_button, self._t("Sync File Dates Hint"))
    ...
    def _connect_signals(self):
        ...
        self.file_list.itemSelectionChanged.connect(self._update_result_folder_button)
        self.open_result_folder_button.clicked.connect(self._on_open_result_folder_clicked)
        self.sync_file_dates_button.clicked.connect(self._on_sync_file_dates_clicked)
        self.shortcut_list_button.clicked.connect(self._on_shortcut_list_clicked)
        ...
        self.toolbar.show_original_only_toggled.connect(self.controller.set_show_original_only)
        ...
        self.property_panel.white_frame_release_requested.connect(
            self.controller.release_custom_white_frame_for_region
        )
        ...
        self.property_panel.italic_changed.connect(self.controller.update_italic)
        self.property_panel.bold_changed.connect(self.controller.update_bold)
        ...
        self.property_panel.char_width_changed.connect(self.controller.update_char_width)
        self.property_panel.angle_changed.connect(self.controller.update_angle)
        self.property_panel.distort_mode_changed.connect(self.controller.update_distort_mode)
        ...
        self.property_panel.toggle_overlay_visibility.connect(lambda state: self.controller.set_display_mask_type('paint', state))
    ...
    def _create_right_panel(self) -> QWidget:
        ...
        self.file_list = FileListView(
            None, self, empty_state_key=FileListView.EDITOR_EMPTY_STATE_TEXT_KEY
        )
        self.file_list.setObjectName("editor_file_list")
        right_layout.addWidget(self.file_list, 1)

        footer = QWidget()
        footer.setObjectName("editor_file_list_footer")
        footer_layout = QVBoxLayout(footer)
        footer_layout.setContentsMargins(0, 4, 0, 0)
        footer_layout.setSpacing(6)

        expanding = QSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self.open_result_folder_button = QPushButton(self._t("Open Result Folder"))
        self.open_result_folder_button.setObjectName("editor_open_result_folder_button")
        self.open_result_folder_button.setProperty("chipButton", True)
        self.open_result_folder_button.setSizePolicy(expanding)
        self.open_result_folder_button.setMinimumHeight(32)
        self.open_result_folder_button.setEnabled(False)
        footer_layout.addWidget(self.open_result_folder_button)

        self.sync_file_dates_button = QPushButton(self._t("Sync File Dates"))
        self.sync_file_dates_button.setObjectName("editor_sync_file_dates_button")
        self.sync_file_dates_button.setProperty("chipButton", True)
        self.sync_file_dates_button.setSizePolicy(expanding)
        self.sync_file_dates_button.setMinimumHeight(32)
        set_hover_hint(self.sync_file_dates_button, self._t("Sync File Dates Hint"))
        footer_layout.addWidget(self.sync_file_dates_button)

        self.shortcut_list_button = QPushButton(self._t("Shortcut List"))
        self.shortcut_list_button.setObjectName("editor_shortcut_list_button")
        self.shortcut_list_button.setProperty("chipButton", True)
        self.shortcut_list_button.setSizePolicy(expanding)
        self.shortcut_list_button.setMinimumHeight(32)
        footer_layout.addWidget(self.shortcut_list_button)
    ...
    def _on_file_remove_requested(self, file_path: str):
        ...
        self._update_result_folder_button()
        
        # 编辑器有自己独立的文件列表，不需要同步到主页的 app_logic

    def _update_result_folder_button(self):
        if self.open_result_folder_button is None or self.file_list is None:
            return
        path, _is_folder = self.file_list.get_selected_entry()
        has_items = self.file_list.topLevelItemCount() > 0
        self.open_result_folder_button.setEnabled(bool(has_items and path))

    def _on_open_result_folder_clicked(self):
        if self.file_list is None:
            return
        path, is_folder = self.file_list.get_selected_entry()
        if not path:
            return
        self.logic.open_selected_result_folder(path, is_folder, parent=self)

    def _on_sync_file_dates_clicked(self):
        synced = 0
        try:
            synced = self.logic.sync_result_file_dates()
        except Exception as e:
            if getattr(self.logic, "logger", None) is not None:
                self.logic.logger.debug(f"Failed to sync result file dates: {e}")
        if synced <= 0:
            return
        toast_manager = self.controller.get_toast_manager()
        if toast_manager is not None:
            toast_manager.show_success(
                self._t("All output file dates have been synchronized to the current time.")
            )

    def _on_shortcut_list_clicked(self):
        from .shortcut_list_dialog import ShortcutListDialog

        dialog = self._shortcut_list_dialog
        ...
        dialog = ShortcutListDialog(self.window(), self._t)
        dialog.destroyed.connect(self._on_shortcut_list_closed)
        self._shortcut_list_dialog = dialog
        dialog.show()

    def _on_shortcut_list_closed(self, *_args):
        self._shortcut_list_dialog = None
