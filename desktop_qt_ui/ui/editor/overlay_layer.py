...
class PixmapOverlayLayer:
    ...
    def _invalidate_view(self) -> None:
        """Force a redraw after same-size pixmap/content changes.

        The inpainted overlay sits under the original image item (opacity 0).
        SmartViewportUpdate + CacheBackground can keep the old pixels until
        the scene is rebuilt (e.g. by switching pages).
        """
        if not self.update_scene:
            return
        if hasattr(self.view, "resetCachedContent"):
            self.view.resetCachedContent()
        image_item = getattr(self.view, "_image_item", None)
