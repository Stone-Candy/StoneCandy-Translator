...
MALGUN_GOTHIC_FAMILIES = ("Malgun Gothic", "맑은 고딕")
PAGE_TITLE_FONT_PX = 19


def malgun_gothic_bold_font(pixel_size: int = PAGE_TITLE_FONT_PX) -> QFont:
    font = QFont()
    font.setFamilies(list(MALGUN_GOTHIC_FAMILIES))
...
def _build_accent_overrides(accent: str, *, dark_theme: bool) -> dict:
    ...
    return {
        ...
        "nav_checked_bg": _rgba(base, 0.42 if dark_theme else 0.21),
...
def apply_native_title_bar_theme(widget: QWidget, theme: str | None = None, logger=None) -> None:
    ...
    try:
        ...
        if logger is not None:
            logger.debug(f"네이티브 제목 표시줄 테마 적용 실패: {exc}")
