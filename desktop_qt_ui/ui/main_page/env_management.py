...
def create_api_rotation_widgets(
    ...
    *,
    ...
    if strategy_key:
        strategy_label = QLabel(self._t("API rotation strategy:").rstrip(":："))
    ...
    def add_slot(index: int):
        ...
        for base_key in (api_key_env, model_env, api_base_env):
            ...
            if not key:
                continue
            value = current_values.get(key, "")
            label = QLabel(_display_env_label(self, base_key, index, include_index=False))
...
def _format_test_connection_error(api_type: str, message: str) -> str:
    ...
    for prefix in ("연결 실패:", "연결 실패：", "连接失败:", "连接失败：", "api connection failed:", "connection failed:"):
    ...
    if is_network_error:
        friendly_message = (
            "연결 오류, 시간 초과 또는 Host 해석 오류가 감지되었습니다.\n"
            "먼저 모델, API 주소, API 키가 올바른지 확인하세요. 설정이 맞다면 네트워크를 확인하고 TUN(가상 네트워크 어댑터 모드)을 켜 보세요."
        )
    elif is_service_error:
        friendly_message = (
            "먼저 모델, API 주소, API 키가 올바른지 확인하세요.\n"
            "설정이 맞다면 API 사이트, 중계 채널, 서버 일시 장애이거나 네트워크가 불안정할 수 있습니다. 잠시 후 다시 시도하거나 API 사이트/채널을 바꿔 보세요."
        )
    else:
        friendly_message = "모델, API 주소, API 키가 올바른지 확인하세요."

    friendly_message += f"\n\nAPI 주소 예시: {_get_api_address_example(api_type)}"
    if raw_message:
...
def on_open_custom_api_params_file(self):
    ...
    try:
        ...
        QMessageBox.warning(self, self._t("Error"), f"설정 파일 생성 실패: {e}")
        return
    ...
    try:
        ...
        QMessageBox.warning(self, self._t("Error"), f"편집기 열기 실패: {e}")
