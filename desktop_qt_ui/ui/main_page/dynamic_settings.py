...
def _selected_api_group_keys(config) -> dict[str, list[str]]:
    ...
    for override_name in ("novelai_ocr", "novelai_secondary_ocr"):
        override_value = _normalize_selected_value(getattr(config.ocr, override_name, "none"))
        if override_value and override_value.lower() != "none":
            selected_ocr_values.append(override_value)
...
def _add_settings_divider(self, parent_layout, title: str, is_sub: bool = False):
    ...
    if is_sub:
        ...
        title_label = QLabel(title if "NovelAI" in title else title.upper())
...
def _finalize_settings_ui(self):
    ...
    if cli_panel and not getattr(self, "_settings_tabs_use_reclassify", False):
        ...
        if cli_layout is not None and isinstance(cli_layout, QFormLayout):
            ...
            unload_models_label = QLabel(label_text)
    ...
    refresh = getattr(self, "refresh_workflow_mode_combo", None)
    if callable(refresh):
        refresh()
    ...
    preset_label = QLabel(self._t("Preset:").rstrip(":："))
...
def _on_setting_changed(self, value, full_key, display_map=None):
    ...
    if full_key in {
        "app.show_colorize_upscale_inpaint_modes",
        "app.show_novelai_mode",
    }:
        refresh = getattr(self, "refresh_workflow_mode_combo", None)
        if callable(refresh):
            refresh()
...
def _create_param_widgets(self, data, parent_layout, prefix=""):
    ...
    for key, value in data.items():
        ...
        if full_key in ["cli.load_text", "cli.rerender_only", "cli.translate_json_only", "cli.template", "cli.generate_and_export", "cli.colorize_only", "cli.upscale_only", "cli.inpaint_only", "cli.novelai_mode", "cli.replace_translation", "cli.replace_translation_mode", "upscale.realcugan_model", "render.gimp_font", "render.font_path", "translator.high_quality_prompt_path", "app.last_open_dir", "app.last_output_path", "app.favorite_folders", "app.current_preset"]:
            continue
        ...
        elif full_key == "app.show_colorize_upscale_inpaint_modes":
            translated = self._t("label_show_colorize_upscale_inpaint_modes")
            label_text = (
                translated
                if translated != "label_show_colorize_upscale_inpaint_modes"
                else "Use Colorize, Upscale, and Cleaning Modes"
            )
        elif full_key == "app.show_novelai_mode":
            translated = self._t("label_show_novelai_mode")
            label_text = translated if translated != "label_show_novelai_mode" else "Use NovelAI Mode"
        ...
        # 特殊处理：upscale_ratio 动态下拉框（必须在 int/float 判断之前）
            ...
            display_value = f"{value:.2f}" if key == "char_width" and isinstance(value, float) else str(value)
        ...
        elif value is None and key in ['tile_size', 'line_spacing', 'letter_spacing', 'char_width', 'font_size', 'psd_font', 'ocr_vl_custom_prompt', 'ai_ocr_custom_prompt']:
            ...
            if key == 'tile_size':
                ...
                widget.setPlaceholderText(self._t("Default: 1.0 (horizontal width scale)"))
                widget.editingFinished.connect(lambda k=full_key, w=widget: self._on_numeric_input_changed(w.text(), k, float))
            elif key == 'font_size':
            ...
            elif full_key in {"ocr.ocr_vl_language_hint", "ocr.novelai_ocr_vl_language_hint"}:
                widget.setMinimumWidth(260)  # OCR语言全称较长，避免被截断
            elif full_key == "cli.overwrite":
