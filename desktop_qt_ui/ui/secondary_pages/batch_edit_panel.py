...
from ui.styles import table_stylesheet
...
class BatchEditPanel(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        status_card.setObjectName("header_card")
    ...
    def _build_scheme_bar(self, parent: QWidget) -> QWidget:
        ...
        layout.setContentsMargins(17, 2, 4, 10)
        layout.setSpacing(8)

        self.scheme_label = QLabel(self._t("Scheme:"), bar)
        self.scheme_combo = ComboBox(bar)
        self.scheme_combo.setMinimumWidth(200)
        self.scheme_combo.currentIndexChanged.connect(self._on_scheme_selected)
        self.new_button = QPushButton(self._t("New"), bar)
        self.rename_button = QPushButton(self._t("Rename"), bar)
        self.duplicate_button = QPushButton(self._t("Duplicate"), bar)
        self.delete_button = QPushButton(self._t("Delete"), bar)
    ...
    def _build_conditions_card(self, parent: QWidget) -> QWidget:
        card = QGroupBox(self._t("Match conditions"), parent)
        card.setObjectName("section_card")
        card.setProperty("role", "conditions")
        self.conditions_card = card
        ...
        self.add_condition_button = QPushButton(self._t("Add condition"), header)
        self.add_condition_button.clicked.connect(lambda: self._add_condition_row())
        self._widen_add_condition_button()
        self.conditions_hint = QLabel(
            self._t("If no conditions are set, the entire range of data in the main list is targeted."), header
        )
        self.conditions_hint.setWordWrap(True)
        self.conditions_hint.setContentsMargins(18, 0, 0, 0)
        header_layout.addWidget(self.logic_combo)
        header_layout.addWidget(self.add_condition_button)
        header_layout.addWidget(self.conditions_hint, 1)
        ...
        self.conditions_empty_label = QLabel(
            self._t("(No conditions set.)"), card
        )
        self.conditions_empty_label.setObjectName("conditions_empty_label")
        self.conditions_empty_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(
            self.conditions_empty_label, 0, Qt.AlignmentFlag.AlignHCenter
        )
        self._sync_conditions_empty()
        return card

    def _build_actions_card(self, parent: QWidget) -> QWidget:
        card = QGroupBox(self._t("Batch actions"), parent)
        card.setObjectName("section_card")
        ...
        self.actions_hint = QLabel(
            self._t("Runs in this order: Set region properties → Replace matched text."),
            card,
        )
    ...
    def _build_preview_card(self, parent: QWidget) -> QWidget:
        ...
        header_layout.setContentsMargins(0, 0, 0, 0)
        header_layout.setSpacing(8)
        self.preview_button = QPushButton(self._t("Preview matches"), header)
        self.preview_button.clicked.connect(self._on_preview)
        self.select_all_button = QPushButton(self._t("Select All"), header)
        self.select_none_button = QPushButton(self._t("Select None"), header)
        self.select_all_button.clicked.connect(lambda: self._set_all_checked(True))
        self.select_none_button.clicked.connect(lambda: self._set_all_checked(False))
        self.match_summary = QLabel("", header)
        self.backup_box = QCheckBox(self._t("Back up each file before writing"), header)
        ...
        self.table.setStyleSheet(table_stylesheet(border_radius=8))
        ...
        self.restore_button = QPushButton(self._t("Restore from backup"), footer)
        ...
        self.apply_button = QPushButton(self._t("Apply to selected"), footer)
    ...
    def _sync_conditions_empty(self) -> None:
        if hasattr(self, "conditions_empty_label"):
            self.conditions_empty_label.setVisible(not self._condition_rows)
    ...
    def _widen_add_condition_button(self) -> None:
        button = getattr(self, "add_condition_button", None)
        if button is None:
            return
        button.setMinimumWidth(0)
        button.setMinimumWidth(max(1, int(round(button.sizeHint().width() * 1.3))))
    ...
    def refresh_ui_texts(self) -> None:
        ...
        self.conditions_card.setTitle(self._t("Match conditions"))
        ...
        self.conditions_hint.setText(
            self._t("If no conditions are set, the entire range of data in the main list is targeted.")
        )
        self.conditions_empty_label.setText(self._t("(No conditions set.)"))
        self.actions_card.setTitle(self._t("Batch actions"))
