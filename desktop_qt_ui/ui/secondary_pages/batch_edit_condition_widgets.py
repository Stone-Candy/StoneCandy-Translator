...
def _close_tool_button(parent=None) -> QPushButton:
    button = QPushButton("✕", parent)
    button.setObjectName("file_item_remove_button")
...
def _style_summary(style: dict, empty_text: str) -> str:
    ...
    if style.get("fontSize"):
        parts.append(str(style["fontSize"]))
    if style.get("ruby"):
        parts.append(str(style["ruby"]))
    if style.get("tcy"):
        parts.append("tcy")
    return " / ".join(parts) if parts else empty_text


class RichTextStyleDialog(QDialog):
    """Compact style picker used by batch-edit rich-text actions."""

    def __init__(self, style: dict, t_func: Callable, parent=None):
        super().__init__(parent)
        self._t = t_func
        self._style = copy.deepcopy(style) if isinstance(style, dict) else {}
        self.setWindowTitle(self._t("Edit Style"))
        self.setModal(True)
        ...
        self.bold_box = QCheckBox(self._t("Bold"), self)
        self.italic_box = QCheckBox(self._t("Italic"), self)
        self.underline_box = QCheckBox(self._t("Underline"), self)
        self.strike_box = QCheckBox(self._t("Strikethrough"), self)
        self.tcy_box = QCheckBox(self._t("TCY"), self)
        self.bold_box.setChecked(bool(self._style.get("bold")))
        self.italic_box.setChecked(bool(self._style.get("italic")))
        self.underline_box.setChecked(bool(self._style.get("underline")))
        self.strike_box.setChecked(bool(self._style.get("strikethrough")))
        self.tcy_box.setChecked(bool(self._style.get("tcy")))
        for box in (self.bold_box, self.italic_box, self.underline_box, self.strike_box, self.tcy_box):
            root.addWidget(box)

        color_row = QHBoxLayout()
        color_row.addWidget(QLabel(self._t("Text Color"), self))
        self.color_picker = ColorPickerWidget(
            dialog_title=self._t("Select color"),
            default_color=str(self._style.get("color") or "#000000"),
            i18n_func=self._t,
            parent=self,
        )
        if self._style.get("color"):
            self.color_picker.set_color(str(self._style["color"]))
        color_row.addWidget(self.color_picker, 1)
        root.addLayout(color_row)

        ruby_row = QHBoxLayout()
        ruby_row.addWidget(QLabel(self._t("Ruby"), self))
        self.ruby_edit = QLineEdit(self)
        self.ruby_edit.setText(str(self._style.get("ruby") or ""))
        ruby_row.addWidget(self.ruby_edit, 1)
        root.addLayout(ruby_row)

        buttons = QHBoxLayout()
        buttons.addStretch(1)
        cancel_button = QPushButton(self._t("Cancel"), self)
        cancel_button.clicked.connect(self.reject)
        ok_button = QPushButton(self._t("OK"), self)
    ...
    def result_style(self) -> dict:
        style: dict[str, Any] = {}
        if self.bold_box.isChecked():
            style["bold"] = True
        if self.italic_box.isChecked():
            style["italic"] = True
        if self.underline_box.isChecked():
            style["underline"] = True
        if self.strike_box.isChecked():
            style["strikethrough"] = True
        if self.tcy_box.isChecked():
            style["tcy"] = True
        color = self.color_picker.get_color()
        if color:
            style["color"] = color
        ruby = self.ruby_edit.text().strip()
        if ruby:
            style["ruby"] = ruby
        return style
...
class _RangeValueEditor(_ValueEditor):
    def __init__(self, integer: bool, t_func: Callable, parent=None):
        ...
        self._separator = QLabel(self._t("to"), self)
...
class _ColorValueEditor(_ValueEditor):
    def __init__(self, t_func: Callable, config_service, with_tolerance: bool, parent=None):
        ...
        self._tolerance_label: Optional[QLabel] = None
        self._tolerance: Optional[QSpinBox] = None
        if with_tolerance:
            self._tolerance_label = QLabel(self._t("Tolerance"), self)
            self._tolerance = QSpinBox(self)
...
class ConditionRow(QWidget):
    ...
    def __init__(self, t_func: Callable, config_service=None, locale_getter=None, parent=None):
        ...
        self.remove_button = _close_tool_button(self)
...
class _ActionCard(QFrame):
    ...
    def __init__(self, title_key: str, t_func: Callable, parent=None):
        super().__init__(parent)
        self.setObjectName("header_card")
        self._t = t_func
        self._title_key = title_key
        self._root = QVBoxLayout(self)
        self._root.setContentsMargins(12, 10, 12, 10)
        self._root.setSpacing(8)
        self.enabled_box = QCheckBox(self._t(title_key), self)
...
class _EntryListActionCard(_ActionCard):
    ...
    def __init__(self, title_key: str, t_func: Callable, parent=None):
        ...
        self.add_button = QPushButton(self._t(self.add_label_key), self.body)
...
class SetFieldsActionCard(_ActionCard):
    ...
    def __init__(self, t_func: Callable, config_service=None, locale_getter=None, parent=None):
        ...
        self.add_button = QPushButton(self._t("Add property"), self.body)
...
class _ReplaceEntry(_ActionEntry):
    ...
    def __init__(self, t_func: Callable, parent=None):
        ...
        if outer is not None:
            outer.removeWidget(self.remove_button)

        row = QWidget(self.content)
        layout = _row(row)
        layout.setSpacing(0)

        find_cell = self._ratio_cell(row)
        find_layout = _row(find_cell)
        self.find_label = QLabel(self._t("Find "), find_cell)
        self.pattern = QLineEdit(find_cell)
        self.pattern.setPlaceholderText(self._t("Text or regular expression"))
        self.pattern.setMinimumWidth(0)
        self.pattern.textChanged.connect(self.changed)
        self.regex = QCheckBox(self._t("Regex"), find_cell)
        self.regex.toggled.connect(self.changed)
        find_layout.addWidget(self.find_label, 0)
        find_layout.addWidget(self.pattern, 1)
        find_layout.addWidget(self.regex, 0)

        arrow_cell = self._ratio_cell(row)
        arrow_layout = _row(arrow_cell)
        self.arrow_label = QLabel("➜", arrow_cell)
        self.arrow_label.setAlignment(
            Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignVCenter
        )
        arrow_layout.addWidget(self.arrow_label, 1)

        replace_cell = self._ratio_cell(row)
        replace_layout = _row(replace_cell)
        self.replace_label = QLabel(self._t("Replace with"), replace_cell)
        self.replace = QLineEdit(replace_cell)
        self.replace.setPlaceholderText(self._t("Supports backreferences like \\1 when regex is on"))
        self.replace.setMinimumWidth(0)
        self.replace.textChanged.connect(self.changed)
        replace_layout.addWidget(self.replace_label, 0)
        replace_layout.addWidget(self.replace, 1)

        close_cell = self._ratio_cell(row)
        close_layout = _row(close_cell)
        close_layout.addStretch(1)
        self.remove_button.setParent(close_cell)
        ...
        layout.addWidget(find_cell, self._COL_FIND)
        layout.addWidget(arrow_cell, self._COL_ARROW)
        layout.addWidget(replace_cell, self._COL_REPLACE)
        layout.addWidget(close_cell, self._COL_CLOSE)
        self.content_layout.addWidget(row)

    @staticmethod
    def _ratio_cell(parent: QWidget) -> QWidget:
        cell = QWidget(parent)
    ...
    def load(self, action: dict) -> None:
        self.pattern.setText(str(action.get("pattern", "") or ""))
        self.regex.setChecked(bool(action.get("regex", False)))
        self.replace.setText(str(action.get("replace", "") or ""))

    def refresh_ui_texts(self) -> None:
        super().refresh_ui_texts()
        self.find_label.setText(self._t("Find "))
        self.pattern.setPlaceholderText(self._t("Text or regular expression"))
        self.regex.setText(self._t("Regex"))
...
class _RichTextEntry(_ActionEntry):
    ...
    def __init__(self, t_func: Callable, parent=None):
        ...
        self.match_style_label = QLabel(self._t("Match rich text"), match_detail)
        ...
        self.match_style_button = QPushButton(self._t("Edit Style"), match_detail)
        self.match_style_button.clicked.connect(self._edit_match_style)
        self.match_summary = QLabel("", match_detail)
        ...
        self.style_button = QPushButton(self._t("Edit Style"), detail)
        self.style_button.clicked.connect(self._edit_style)
        self.summary = QLabel("", detail)
        detail_layout.addWidget(self.style_button)
        detail_layout.addWidget(self.summary, 1)
        self.content_layout.addWidget(detail)

        self.hint = QLabel(self._t("Leave the pattern empty to target the whole region"), self.content)
    ...
    def _edit_style(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self._style = dialog.result_style()
            self._update_summary()
            self.changed.emit()
    ...
    def _edit_match_style(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self._match_style = dialog.result_style()
            self._update_summary()
            self.changed.emit()
