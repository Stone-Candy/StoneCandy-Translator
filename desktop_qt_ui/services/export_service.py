...
class ExportService:
    ...
    def _save_temp_inpainted_image(
        ...
        try:
            ...
            save_pil_image(
                ...
                stealth_source_path=stealth_source_path,
            )
            self.logger.info(f"내보내기 재사용용 임시 복구 이미지를 썼습니다: {temp_inpainted_path}")
    ...
    def _persist_backend_inpainted_image(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"원본 이미지 메타데이터를 읽지 못했습니다. 저장은 계속하지만 ICC는 상속하지 않습니다: {source_image_path}, error={metadata_error}")
                source_image = None
            save_pil_image(
                save_image,
                inpainted_path,
                ...
                stealth_source_path=source_image_path,
            )
            self.logger.info(f"내보낸 복구 이미지를 다시 썼습니다: {inpainted_path}")
            return inpainted_path
        except Exception as e:
            self.logger.warning(f"내보낸 복구 이미지를 다시 쓰지 못했습니다: {e}")
            return None
        finally:
    ...
    def get_output_directory(self) -> Optional[str]:
        ...
        if global_dir and os.path.exists(global_dir):
            self.logger.info(f"전역 출력 폴더 사용: {global_dir}")
            return global_dir
                    ...
                    if hasattr(controller, 'main_view_widgets'):
                        ...
                        if output_entry:
                            ...
                            if output_dir and os.path.exists(output_dir):
                                self.logger.info(f"출력 폴더를 찾았습니다: {output_dir}")
                                # 更新全局存储
                                set_global_output_directory(output_dir)
                                return output_dir
                            elif output_dir:
                                self.logger.warning(f"출력 폴더가 없습니다: {output_dir}")
                            break
                    break
                    
        except Exception as e:
            self.logger.warning(f"출력 폴더를 가져올 수 없습니다: {e}")
    ...
    def _perform_backend_render_export(self, image: Image.Image, regions_data: List[Dict[str, Any]],
                                     ...
                                     save_inpainted_only: bool = False,
                                     editor_inpainted_image: Optional[Any] = None,
                                     persist_backend_inpainted: bool = True):
        """在后台线程中执行后端渲染导出"""
        import os
        ...
        try:
            self.logger.info(f"이미지 내보내기 시작: {output_path}")
            ...
            self._save_temp_inpainted_image(
                temp_image_path,
                editor_inpainted_image,
                image.size,
                source_image=image,
                stealth_source_path=source_image_path,
            )
            ...
            rendered_image = self._execute_backend_render(
                ...
                source_image_path,
                save_inpainted_only,
                persist_backend_inpainted,
            )
            
            if not rendered_image:
                raise Exception("백엔드 렌더링이 결과를 만들지 못했습니다")
            ...
            self.logger.info(f"이미지를 내보냈습니다: {output_path}")
            
            if success_callback:
                success_callback(f"图片已导出到: {output_path}")

        except Exception as e:
            error_msg = f"백엔드 렌더링 내보내기 실패: {e}"
    ...
    def _read_existing_image_data(self, json_path: str, image_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"기존 JSON을 읽지 못해 전처리 플래그를 이어받지 못합니다: {json_path}: {e}")
            return {}
    ...
    def _preserve_existing_preprocess_flags(
        ...
        if not target_data.get('upscale_ratio'):
            ...
            if existing_upscale_ratio:
                ...
                self.logger.info(f"기존 업스케일 정보를 유지합니다: ratio={existing_upscale_ratio}, upscaler={existing_upscaler}")
        ...
        if not target_data.get('colorizer'):
            ...
            if existing_colorizer and str(existing_colorizer).lower() != 'none':
                ...
                self.logger.info(f"기존 채색 정보를 유지합니다: colorizer={existing_colorizer}")
    ...
    def _save_regions_data_internal(
        ...
        if config:
            ...
            if upscale_ratio:
                ...
                self.logger.info(f"JSON에 업스케일 정보를 기록합니다: ratio={upscale_ratio}, upscaler={upscaler}")
            ...
            if colorizer and colorizer != 'none':
                ...
                self.logger.info(f"JSON에 채색 정보를 기록합니다: colorizer={colorizer}")

        # 편집기 재저장 시 regions만 갈아끼우면 last_export_dir이 사라질 수 있다.
        # preprocess 플래그 보존과 별도로, 항상 기존 last_export_dir을 유지한다.
        ...
        if existing_image_data:
            preserved_export_dir = existing_image_data.get("last_export_dir")
            if (
                isinstance(preserved_export_dir, str)
                and preserved_export_dir
                and "last_export_dir" not in formatted_data[image_key]
            ):
                formatted_data[image_key]["last_export_dir"] = preserved_export_dir
        
        # 如果有蒙版数据，则添加到JSON中
        if mask is not None:
            self.logger.info("내보내기 JSON에 미리 계산된 마스크(편집된 refined mask)를 넣습니다.")
            ...
            self.logger.info("마스크를 저장했습니다(base64). 정제됨으로 표시되어 백엔드는 마스크 최적화를 건너뜁니다")
        if skip_text_replacements:
            formatted_data[image_key]['skip_text_replacements'] = True

        # 添加调试信息
        self.logger.info(f"영역 데이터를 저장합니다: {json_path}")
        self.logger.info(f"영역 개수: {len(save_data)}")
    ...
    def _save_rendered_image(
        ...
        try:
            ...
            stealth_kw = {'stealth_source_path': stealth_source_path}
            if output_lower.endswith(('.jpg', '.jpeg')):
                ...
                try:
                    ...
                    self.logger.warning("HEIC/HEIF 형식은 pillow-heif가 필요합니다. PNG로 폴백합니다")
                    # 修改输出路径为 PNG
            ...
            if not os.path.exists(temp_output_path):
                raise Exception(f"임시 파일을 만들지 못했습니다: {temp_output_path}")
            
            # 原子性替换
            os.replace(temp_output_path, output_path)
            self.logger.info(f"이미지를 저장했습니다: {output_path}")
    ...
    def _prepare_translator_params(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if font_family_value:
            ...
            self.logger.info(f"폰트 family 전달: {font_family_value}")
        else:
            self.logger.info("전역 폰트가 없어 영역 폰트 또는 백엔드 기본 폰트를 사용합니다")
        
        # 提取输出格式
        output_format = self.get_output_format_from_config(config)
        if output_format:
            translator_params['format'] = output_format
            self.logger.info(f"출력 형식 설정: {output_format}")
        
        # 提取并传递GPU配置
        cli_config = config.get('cli', {})
        if 'use_gpu' in cli_config:
            translator_params['use_gpu'] = cli_config['use_gpu']
            self.logger.info(f"GPU 설정: use_gpu={cli_config['use_gpu']}")
        ...
        self.logger.info("번역기를 none으로 설정하고 load_text 모드를 켜서 번역 단계를 건너뛰고 바로 렌더링합니다")
    ...
    def _execute_backend_render(self, image_path: str, regions_json_path: str,
                              ...
                              save_inpainted_only: bool = False,
                              persist_backend_inpainted: bool = True) -> Optional[Image.Image]:
        """执行后端渲染"""
        image = None
        try:
            ...
            self.logger.info(f"PSD 내보내기 설정: export_editable_psd={cli_cfg.export_editable_psd}, psd_font={cli_cfg.psd_font}, psd_launch_photoshop={cli_cfg.psd_launch_photoshop}")
            ...
            try:
                ...
                if save_inpainted_only:
                    ...
                    if hasattr(ctx, 'img_inpainted') and ctx.img_inpainted is not None:
                        ...
                        self.logger.info("복구된 이미지(inpainted)를 반환합니다")
                    else:
                        self.logger.warning("ctx.img_inpainted가 없어 result로 폴백합니다")
                ...
                if cfg.cli.export_editable_psd and not save_inpainted_only:
                        try:
                            ...
                            default_font = cfg.cli.psd_font or getattr(cfg.render, 'font_family', None)
                            line_spacing = cfg.render.line_spacing if hasattr(cfg.render, 'line_spacing') else None
                            script_only = not cfg.cli.psd_launch_photoshop
                            ...
                            self.logger.info(f"PSD 내보내기 시작: {psd_path}")
                            self.logger.info(f"이미지 경로로 inpainted 검색: {image_path_for_psd}")
                            photoshop_export(psd_path, ctx, default_font, image_path_for_psd, False, None, line_spacing, script_only)
                            self.logger.info(f"✅ [PSD] 편집 가능한 PSD를 내보냈습니다: {os.path.basename(psd_path)}")
                            
                            if progress_callback:
                                progress_callback(f"已导出PSD: {os.path.basename(psd_path)}")
                        except Exception as psd_err:
                            self.logger.error(f"PSD 내보내기 실패: {psd_err}")
                            import traceback
                            self.logger.error(traceback.format_exc())
                ...
                if hasattr(ctx, 'result') and ctx.result is not None:
                    try:
                        ...
                        self.logger.error(f"ctx.result를 닫지 못했습니다: {close_error}")
                ...
                if image:
                    try:
                        ...
                        self.logger.error(f"입력 이미지를 닫지 못했습니다: {close_error}")
                
                return result_image

            except Exception as translate_error:
                self.logger.error(f"translator.translate 실행 실패: {translate_error}")
                self.logger.error(f"오류 유형: {type(translate_error).__name__}")
                import traceback
                self.logger.error(f"전체 스택:\n{traceback.format_exc()}")
                raise
            finally:
                shutdown_event_loop(loop, logger=self.logger, label="backend export loop")

        except Exception as e:
            self.logger.error(f"백엔드 렌더링 중 오류: {type(e).__name__}: {e}")
            import traceback
            self.logger.error(f"전체 스택:\n{traceback.format_exc()}")
            raise
        finally:
            ...
            if image:
                try:
                    ...
                    self.logger.error(f"finally에서 입력 이미지를 닫지 못했습니다: {close_error}")
    ...
    def export_regions_json(self, regions_data: List[Dict[str, Any]], output_path: str, config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            self.logger.info(f"영역 데이터를 내보냈습니다: {output_path}")
            return True
        except Exception as e:
            self.logger.error(f"영역 데이터 내보내기 실패: {e}")
            return False
