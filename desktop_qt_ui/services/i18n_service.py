...
class I18nManager:
    ...
    def __init__(self, locale_dir: str = "locales", fallback_locale: str = "zh_CN", config_language: str = "auto"):
        ...
        self.logger.info(f"Initialized i18n manager, config language: {config_language}, current language: {self.current_locale}")
    ...
    def _detect_system_locale(self) -> str:
        ...
        try:
            ...
            self.logger.warning(f"시스템 언어 감지 실패: {e}")
    ...
    def _load_locale_translation(self, locale_code: str):
        ...
        try:
            ...
            if os.path.exists(translation_file):
                ...
                self.logger.debug(f"번역 파일 로드: {translation_file}")
            else:
                # 如果翻译文件不存在，创建空的翻译字典
            ...
            self.logger.error(f"번역 파일 로드 실패 {locale_code}: {e}")
            self.translations[locale_code] = {}
    ...
    def _create_base_translation_file(self, locale_code: str):
        ...
        try:
            ...
            self.logger.info(f"기본 번역 파일 생성: {translation_file}")
            
        except Exception as e:
            self.logger.error(f"기본 번역 파일 생성 실패: {e}")
    ...
    def set_locale(self, locale_code: str) -> bool:
        ...
        if locale_code not in self.available_locales:
            self.logger.warning(f"지원하지 않는 언어: {locale_code}")
            return False
        ...
        self.logger.info(f"언어 전환: {old_locale} -> {locale_code}")
        return True
    ...
    def translate(self, key: str, locale_code: str = None, **kwargs) -> str:
        ...
        # 格式化翻译（支持参数替换）
        # 注意：en_US 等语言 key 与 value 可能相同，仍需执行 format
        if kwargs:
            try:
                translation = translation.format(**kwargs)
            except Exception as e:
                self.logger.warning(f"번역 포맷 실패 {key}: {e}")
    ...
    def save_translations(self, locale_code: str = None) -> bool:
        ...
        try:
            ...
            self.logger.error(f"번역 저장 실패: {e}")
            return False
    ...
    def _save_locale_translation(self, locale_code: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"번역 파일 저장: {translation_file}")
            return True
            
        except Exception as e:
            self.logger.error(f"번역 파일 저장 실패 {locale_code}: {e}")
            return False
    ...
    def export_missing_keys(self, locale_code: str, output_file: str) -> bool:
        ...
        try:
            ...
            if missing_keys:
                ...
                self.logger.info(f"누락된 번역 {len(missing_keys)}개를 내보냅니다: {output_file}")
                return True
            else:
                self.logger.info(f"언어 {locale_code}에는 누락된 번역이 없습니다")
                return True
                
        except Exception as e:
            self.logger.error(f"누락된 번역 내보내기 실패: {e}")
            return False
