...
class ConfigService(QObject):
    ...
    def __init__(self, root_dir: str):
        ...
        try:
            ...
            self.logger.error(f"Failed to create local config template files: {exc}")
        self.logger.debug(f"Default config: {os.path.basename(self.default_config_path)}")
        self.logger.debug(f"User config: {os.path.basename(self.user_config_path)}")
        self.logger.debug(f"Default config exists: {os.path.exists(self.default_config_path)}")
        self.logger.debug(f"User config exists: {os.path.exists(self.user_config_path)}")
        if hasattr(sys, '_MEIPASS'):
            self.logger.debug(f"Packaged environment, sys._MEIPASS = {sys._MEIPASS}")
    ...
    def get_missing_runtime_api_requirements(
        ...
        novelai_mode = bool(getattr(getattr(config, "cli", None), "novelai_mode", False))

        def _ocr_override_disabled(value) -> bool:
            return str(getattr(value, "value", value) or "").strip().lower() in {"", "none"}

        if novelai_mode:
            novelai_ocr = getattr(config.ocr, "novelai_ocr", "none")
            novelai_secondary = getattr(config.ocr, "novelai_secondary_ocr", "none")
            primary_ocr = (
                getattr(config.ocr, "ocr", None)
                if _ocr_override_disabled(novelai_ocr)
                else novelai_ocr
            )
            checks = [
                ("ocr", "novelai_ocr" if not _ocr_override_disabled(novelai_ocr) else "ocr", primary_ocr),
                ("colorizer", "colorizer", getattr(config.colorizer, "colorizer", None)),
                ("render", "renderer", getattr(config.render, "renderer", None)),
            ]
            if not _ocr_override_disabled(novelai_secondary):
                checks.append(("ocr", "novelai_secondary_ocr", novelai_secondary))
        else:
            checks = [
    ...
    def load_config_file(self, config_path: str) -> bool:
        ...
        try:
                    ...
                    try:
                        if isinstance(value, dict) and key in target and isinstance(target[key], dict):
                            ...
                            try:
                                ...
                                self.logger.warning(f"Invalid config key '{current_path}': {value}, using default: {old_value}")
                    except Exception as e:
                        error_keys.append((current_path, value, str(e)))
                        self.logger.warning(f"Failed to load config key '{current_path}': {e}, keeping default")
            ...
            try:
                ...
                self.logger.error(f"Config validation failed, using defaults: {final_err}")
                self.current_config = AppSettings()
            
            # 报告错误的键
            if error_keys:
                self.logger.warning(f"{len(error_keys)} invalid config keys were replaced with defaults:")
            ...
            self.logger.debug(f"Loaded config: {os.path.basename(config_path)}")
            config_dict = self.current_config.model_dump()
            self.config_changed.emit(config_dict)
            return True
            
        except json.JSONDecodeError as e:
            self.logger.error(f"Config file JSON is invalid: {e}, using defaults")
            self.current_config = AppSettings()
            return False
        except Exception as e:
            ...
            for save_path in save_paths:
                ...
                if is_default_config and not hasattr(sys, '_MEIPASS'):
                    ...
                    if 'render' not in config_dict:
                        config_dict['render'] = {}
                    config_dict['render']['font_family'] = ''
                    config_dict['render']['disable_system_fonts'] = False
                ...
                try:
                    ...
                    self.logger.error(f"Failed to save config ({os.path.basename(save_path)}): {e}")
            
            if success_count > 0:
                self.config_path = self.user_config_path
                return True
            else:
                self.logger.error("Failed to save all config files")
                return False
    ...
    def reload_config(self):
        ...
        """
        self.logger.info("Force-reloading config...")
        
        # 1. 重新加载 .env 文件到 os.environ。翻译引擎会自动从此读取。
        load_dotenv(self.env_path, override=True)
        self.logger.info(f".env reloaded from {self.env_path}; environment variables updated.")
    ...
    def reload_from_disk(self):
        ...
        if self.config_path and os.path.exists(self.config_path):
            self.logger.debug(f"Reloaded config from disk: {os.path.basename(self.config_path)}")
            self.load_config_file(self.config_path)
        else:
            self.logger.warning("Cannot reload config: config_path is unset or the file does not exist.")
    ...
    def set_config(self, config: AppSettings) -> None:
        ...
        self.logger.debug("Config updated; notifying listeners...")
        config_dict = self.current_config.model_dump()
        self.config_changed.emit(config_dict)
    ...
    def save_env_var(self, key: str, value: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to save environment variables: {e}")
            return False
    ...
    def save_env_vars(self, env_vars: Dict[str, str]) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to batch-save environment variables: {e}")
            import traceback
    ...
    def _load_configs_with_priority(self):
        ...
        if os.path.exists(self.default_config_path):
            self.logger.info(f"Loading default config: {self.default_config_path}")
            self.load_config_file(self.default_config_path)
        else:
            self.logger.warning(f"Default config not found: {self.default_config_path}")
        
        # 2. 再加载用户配置（如果存在），覆盖默认配置
        if os.path.exists(self.user_config_path):
            self.logger.info(f"Loading user config: {self.user_config_path}")
            self.load_config_file(self.user_config_path)
            self.config_path = self.user_config_path
        else:
            self.logger.info(f"User config not found: {self.user_config_path}")
            # 如果用户配置不存在，从默认配置创建一份
            if os.path.exists(self.default_config_path):
                self.logger.info("Creating user config from default config")
                try:
                    # 复制默认配置到用户配置位置
                    ...
                    self.logger.info(f"User config created: {self.user_config_path}")
                    self.config_path = self.user_config_path
                except Exception as e:
                    self.logger.error(f"Failed to create user config: {e}")
    ...
    def _sync_user_config(self):
        ...
        if not os.path.exists(self.default_config_path):
            self.logger.warning("Default config not found, skipping sync")
            return
        
        if not os.path.exists(self.user_config_path):
            self.logger.info("User config not found, skipping sync")
            return
        ...
        try:
            ...
            if synced_data != user_data:
                self.logger.info("Config structure changed; syncing user config")
                with open(self.user_config_path, 'w', encoding='utf-8') as f:
                    json.dump(synced_data, f, indent=2, ensure_ascii=False)
                self.logger.info("User config sync complete")
                
        except Exception as e:
            self.logger.error(f"Failed to sync user config: {e}")
