...
try:
    ...
    logging.warning(f"Failed to import OCR backend: {e}")
    OCR_AVAILABLE = False
...
class OcrService:
    ...
    def __init__(self, config_service=None):
        ...
        self.logger.info(f"OCR service initialized, device: {self.device}")
    ...
    def _get_current_config(self) -> OcrConfig:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"OCR 설정 파싱 실패, 기본 설정을 사용합니다: {parse_error}")
            ...
            self.logger.error(f"OCR 설정을 가져오지 못해 기본 설정을 사용합니다: {e}")
            return OcrConfig(
                ocr=Ocr.ocr48px,
                min_text_length=0,
                ignore_bubble=0,
    ...
    async def prepare_model(self, ocr_type: Optional[Ocr] = None):
        ...
        if not OCR_AVAILABLE:
            raise RuntimeError("OCR 백엔드 모듈을 사용할 수 없습니다")
        ...
        try:
            ...
            self.logger.info(f"OCR 모델 준비 완료: {ocr_to_use.value}")
        except Exception as e:
            self.logger.error(f"OCR 모델 준비 실패: {e}")
            raise
    ...
    def _region_to_quadrilateral(self, region: Dict[str, Any], image_shape: Tuple[int, int]) -> Quadrilateral:
        ...
        try:
            ...
            self.logger.error(f"영역 변환 실패: {e}")
            return None
    ...
    def _extract_region_image(self, image: np.ndarray, region: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.logger.error(f"영역 이미지 추출 실패: {e}")
            return None
    ...
    async def _get_yolo_detector(self):
        ...
        try:
            ...
            self.logger.warning("YOLOOBBDetector 사용할 수 없습니다")
            return None
    ...
    async def recognize_region(self, image: np.ndarray, region: Dict[str, Any], 
        ...
        try:
            ...
            if results:
                ...
                self.logger.warning("OCR 인식 결과가 없습니다")
                return None
                
        except Exception as e:
            self.logger.error(f"OCR 인식 실패: {e}")
            return None
    ...
    async def recognize_multiple_regions(self, image: np.ndarray, regions: List[Dict[str, Any]], 
        ...
        try:
            ...
            self.logger.error(f"일괄 OCR 인식 실패: {e}")
            return [None] * len(regions)
    ...
    def set_model(self, model_name: str):
        ...
        try:
            ...
            if hasattr(Ocr, model_name):
                ...
                self.logger.info(f"name으로 OCR 모델 설정: {model_name}")
                return
            ...
            for ocr_model in Ocr:
                if ocr_model.value == model_name:
                    ...
                    self.model_prepared = False  # 重置模型准备状态
                    self.logger.info(f"value로 OCR 모델 설정: {model_name} -> {ocr_model.name}")
                    return
                    
            # 如果都没找到，记录警告
            self.logger.warning(f"OCR 모델을 찾지 못했습니다: {model_name}，현재 설정을 유지합니다")
        except Exception as e:
            self.logger.error(f"OCR 모델 설정 중 오류: {e}")
    ...
    def set_config(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(self.default_config, key):
                ...
                self.logger.info(f"OCR 설정 갱신: {key} = {value}")
