...
_DEFAULT_SCHEMES_YAML = """# Batch management schemes
# Each scheme = match conditions (filter regions) + batch actions (edit matches)
#
# match.logic: all | any
# match.conditions[].field and op follow the Batch Management page dropdowns
#
# actions always run as set_fields -> replace_text -> rich_text
# because changing text clears styling on the changed range.
# Multiple replace_text / rich_text entries keep their written order.

schemes:
  - name: "Example: tighten vertical small-text spacing"
    enabled: false
    comment: "Edit the conditions and actions before enabling"
    match:
      logic: all
      conditions:
        - field: direction
...
def get_schemes_path() -> str:
    return os.path.join(BASE_PATH, "examples", "batch_edit_schemes.yaml")
