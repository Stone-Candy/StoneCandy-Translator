...
class EraseConfigService:
    ...
    def __init__(self):
        ...
        self.logger.info("지우기 알고리즘 설정 서비스 초기화 완료")
    ...
    def load_config_from_file(self, config_path: str) -> bool:
        ...
        try:
            ...
            if inpainter_data:
                ...
                self.logger.info(f"설정 파일에서 지우기 알고리즘을 불러왔습니다: {self.current_config.inpainter.value}")
                return True
            else:
                self.logger.warning("설정 파일에 inpainter 설정이 없습니다")
                return False
    ...
    def save_config_to_file(self, config_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"설정을 저장했습니다: {config_path}")
            return True
    ...
    def set_algorithm(self, algorithm: InpainterType):
        ...
        self.logger.info(f"지우기 알고리즘 전환: {algorithm.value}")
    
    def set_inpainting_size(self, size: int):
        """设置修复尺寸"""
        if size < 512 or size > 4096:
            raise ValueError("복원 크기는 512-4096 사이여야 합니다")
        self.current_config.inpainting_size = size
        self.logger.info(f"복구 크기 설정: {size}")
    ...
    def set_precision(self, precision: InpaintPrecision):
        ...
        if not self.algorithm_info[self.current_config.inpainter].supports_precision:
            self.logger.warning(f"현재 알고리즘은 정밀도 설정을 지원하지 않습니다: {self.current_config.inpainter.value}")
            return
        self.current_config.inpainting_precision = precision
        self.logger.info(f"복구 정밀도 설정: {precision.value}")
