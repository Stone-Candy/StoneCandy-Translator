...
class ClipboardManager:
    ...
    def __init__(self):
        ...
        self.slots: dict = {}  # 번호별 스타일 슬롯 (Ctrl+숫자 복사 / 숫자 적용용)
    ...
    def copy_to_slot(self, slot: str, data: Any):
        """번호 슬롯에 데이터 복사 (기본 clipboard_data와 별개의 독립 슬롯)."""
        self.slots[slot] = copy.deepcopy(data)
        self.logger.debug(f"Data copied to clipboard slot '{slot}'")

    def paste_from_slot(self, slot: str) -> Any:
        """번호 슬롯에서 데이터 꺼내기."""
        data = self.slots.get(slot)
        if data is not None:
            return copy.deepcopy(data)
        return None

    def has_slot_data(self, slot: str) -> bool:
        """해당 슬롯에 데이터가 있는지 확인."""
...
class EditorStateManager(QObject):
    ...
    def copy_to_slot(self, slot: str, data: Any):
        """번호 슬롯에 데이터 복사."""
        self.clipboard.copy_to_slot(slot, data)

    def paste_from_slot(self, slot: str) -> Any:
        """번호 슬롯에서 데이터 꺼내기."""
        return self.clipboard.paste_from_slot(slot)
