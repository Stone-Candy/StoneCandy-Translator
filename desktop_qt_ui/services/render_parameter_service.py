...
class RenderParameters:
    ...
    char_width: float = 1.0  # 글자 가로폭 배율
...
class RenderParameterService:
    ...
    def __init__(self):
        ...
        self.logger.info("Render parameter service initialized")
    ...
    def calculate_default_parameters(self, region_data: Dict[str, Any]) -> RenderParameters:
        ...
        try:
            ...
            self.logger.debug(f"기본 파라미터 계산: 크기={width}x{height}, 글꼴={font_size}, 방향={direction}")
            return params
            
        except Exception as e:
            self.logger.error(f"기본 파라미터 계산 실패: {e}")
            # 从配置服务获取默认参数
            config = self.config_service.get_config()
            render_fields = RenderParameters.__dataclass_fields__.keys()
            global_render_config = config.render.model_dump()
    ...
    def get_region_parameters(self, region_index: int, region_data: Dict[str, Any] = None) -> RenderParameters:
        ...
        if region_index in self.region_parameters:
            ...
            if region_data:
                ...
                # 行/字间距/가로비율: region_data에 없으면 기본값으로 되돌린다.
                # undo 시 키가 삭제되어도 캐시에 이전 값이 남으면 Ctrl+Z 후 화면이
                # 갱신되지 않는 문제가 생긴다(페이지 전환 시 clear_cache로만 복구됨).
                if 'line_spacing' in region_data and region_data['line_spacing'] is not None:
                    params.line_spacing = region_data['line_spacing']
                else:
                    params.line_spacing = default_params.line_spacing
                if 'letter_spacing' in region_data and region_data['letter_spacing'] is not None:
                    ...
                    params.letter_spacing = default_params.letter_spacing
                if 'char_width' in region_data and region_data['char_width'] is not None:
                    params.char_width = region_data['char_width']
                else:
                    params.char_width = default_params.char_width

                # 字体样式: 키가 없으면 기본값으로 되돌림.
                # undo 시 italic 키가 삭제돼도 캐시에 True가 남으면 화면이 안 돌아온다.
                if 'bold' in region_data:
                    params.bold = bool(region_data['bold'])
                else:
                    params.bold = default_params.bold
                if 'italic' in region_data:
                    params.italic = bool(region_data['italic'])
                else:
                    params.italic = default_params.italic
                if 'underline' in region_data:
                    params.underline = bool(region_data['underline'])
                else:
                    params.underline = default_params.underline
                if 'font_weight' in region_data and region_data['font_weight'] is not None:
                    params.font_weight = region_data['font_weight']
                else:
                    params.font_weight = default_params.font_weight

                if region_data.get('font_family'):
        ...
        if region_data:
            ...
            if 'char_width' in region_data and region_data['char_width'] is not None:
                calculated_params.char_width = region_data['char_width']
            else:
                calculated_params.char_width = default_params.char_width
            ...
            # 字体样式: 키가 없으면 기본값 복원 (Ctrl+Z 후 캐시 잔존 방지)
            if 'bold' in region_data:
                calculated_params.bold = bool(region_data['bold'])
            else:
                calculated_params.bold = default_params.bold
            if 'italic' in region_data:
                calculated_params.italic = bool(region_data['italic'])
            else:
                calculated_params.italic = default_params.italic
            if 'underline' in region_data:
                calculated_params.underline = bool(region_data['underline'])
            else:
                calculated_params.underline = default_params.underline
            if 'font_weight' in region_data and region_data['font_weight'] is not None:
                calculated_params.font_weight = region_data['font_weight']
            else:
                calculated_params.font_weight = default_params.font_weight
    ...
    def set_region_parameters(self, region_index: int, parameters: RenderParameters):
        ...
        self.logger.debug(f"영역 설정 {region_index} 의 렌더 파라미터")
    ...
    def update_region_parameter(self, region_index: int, param_name: str, value: Any):
        ...
        if hasattr(self.region_parameters[region_index], param_name):
            ...
            self.logger.debug(f"영역 갱신 {region_index} 파라미터 {param_name} = {value}")
        else:
            self.logger.warning(f"알 수 없는 파라미터: {param_name}")
    
    def apply_preset(self, region_index: int, preset_name: str) -> bool:
        """应用预设参数到指定区域"""
        if preset_name not in self.presets:
            self.logger.warning(f"프리셋을 찾지 못했습니다: {preset_name}")
            return False
        
        preset_params = copy.deepcopy(self.presets[preset_name].parameters)
        self.set_region_parameters(region_index, preset_params)
        self.logger.info(f"프리셋 적용 '{preset_name}'  → 영역 {region_index}")
        return True
    ...
    def create_custom_preset(self, name: str, description: str, parameters: RenderParameters):
        ...
        self.logger.info(f"사용자 지정 프리셋 생성: {name}")
    ...
    def export_parameters_for_backend(self, region_index: int, region_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # region_data 값이 있으면 최우선. 키가 없으면 위에서 채운 params(기본값 포함) 유지.
        # 캐시에 남은 이전 값을 다시 덮어쓰지 않도록, 없는 키는 기본값으로 명시 복구한다.
        if region_data:
            default_params = self.get_default_parameters()
            if 'line_spacing' in region_data and region_data['line_spacing'] is not None:
                backend_params['line_spacing'] = region_data['line_spacing']
            else:
                backend_params['line_spacing'] = default_params.line_spacing
            if 'letter_spacing' in region_data and region_data['letter_spacing'] is not None:
                backend_params['letter_spacing'] = region_data['letter_spacing']
            else:
                backend_params['letter_spacing'] = default_params.letter_spacing
            if 'char_width' in region_data and region_data['char_width'] is not None:
                backend_params['char_width'] = region_data['char_width']
            else:
                backend_params['char_width'] = default_params.char_width
    ...
    def import_parameters_from_json(self, region_index: int, json_data: Dict[str, Any]):
        ...
        try:
            ...
            if valid_params:
                ...
                self.logger.debug(f"JSON에서 영역 가져오기 {region_index}  파라미터")
                return True
            else:
                self.logger.warning("JSON에 유효한 렌더 파라미터가 없습니다")
                return False
                
        except Exception as e:
            self.logger.error(f"파라미터 가져오기 실패: {e}")
            return False
    ...
    def copy_parameters(self, from_region: int, to_region: int):
        ...
        if from_region in self.region_parameters:
            ...
            self.logger.info(f"파라미터 복사: 영역 {from_region}  → 영역 {to_region}")
            return True
        return False
    ...
    def reset_region_parameters(self, region_index: int):
        ...
        if region_index in self.region_parameters:
            ...
            self.logger.info(f"영역 초기화 {region_index}  파라미터")
    ...
    def get_parameter_summary(self, region_index: int) -> Dict[str, str]:
        ...
        return {
            ...
            "글자 가로폭": f"{params.char_width:.1f}배",
